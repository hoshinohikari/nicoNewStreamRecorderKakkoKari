...
internal class app
{
    ...
    {
        var a = util.getRegGroup("as32df5gh", "\\d([^0-9]).(.)");
        Console.WriteLine(a);
...
//		util.debugWriteLine(jarpath);
        ...
        if (name.Length + dirPath.Length > 234) return new[] { null, name + " " + dirPath };
        ...
        for (var i = 0; i < 1000000; i++)
        {
            ...
            {
                var _existFile = util.existFile(files, "_ts_(\\d+h\\d+m\\d+s)_" + i, name);
                if (_existFile != null)
                {
    ...
    public static string getOkFileName(string name)
    {
        ...
        foreach (var s in replaceCharacter) name = name.Replace(s, "_");
        return name;
    }
    ...
    private static string getFileName(string host, string group, string title, string lvId, string communityNum,
        config cfg)
    {
...
//			string[] ret = {dirPath, dirPath + "/" + name + "_" + "ts" + (i - 1).ToString()};
    ...
    */
    public static string getPageSource(string _url, CookieContainer container = null, string referer = null,
        bool isFirstLog = true, int timeoutMs = 5000)
    {
        timeoutMs = 5000;
        /*
        string a = "";
...
//		var a = container.GetCookieHeader(new Uri(_url));
        debugWriteLine("getfilebyte " + url);
        for (var i = 0; i < 1; i++)
            try
            {
            ...
            {
                debugWriteLine("getfile error " + url + e.Message + e.StackTrace);
                //				System.Threading.Thread.Sleep(3000);
            }
...
//		var files = Directory.GetFiles(dirPath);
        foreach (var f in files)
        {
            ...
            var _reg = getRegGroup(_f.Substring(startWith.Length), reg);
...
//		var dotSecond = ((int)((seconds % 1) * 10)).ToString("0");
    ...
    public static int getPageType(string res)
    {
        ...
        if (res.IndexOf("isFollowerOnly&quot;:true") > -1 && res.IndexOf("isFollowed&quot;:false") > -1) return 4;
        if (data.IndexOf("webSocketUrl&quot;:&quot;ws") == -1 &&
            status == "ENDED") return 2;

        if (status == "ENDED" && res.IndexOf(" onclick=\"Nicolive.WatchingReservation") > -1) return 9;
        ...
        if (getRegGroup(res, "(チャンネル会員限定番組です。<br>)") != null) return 4;
    ...
    public static string getUserName(string userId, out bool isFollow, CookieContainer container)
    {
        ...
        var name = getRegGroup(res, "<meta property=\"og:title\" content=\"(.+?)\">");
        if (name == null)
            name = getRegGroup(res, "<meta property=\"og:title\" content=\"(.+?)さんのユーザーページ\">");
...
//			util.debugWriteLine(container.GetCookieHeader(new Uri(url)) + util.getMainSubStr(isSub, true));
        ...
        if (res == null)
        {
            ...
            res = getPageSource(url, cc);
            if (res == null) return null;
            isFollow = res.IndexOf("<h2 class=\"pageHeader_title\">コミュニティにフォローリクエストを送る</h2>") == -1 &&
                       getRegGroup(res, "<p class=\"error_description\">[\\s\\S]*?(コミュニティフォロワー)ではありません。") == null &&
...
//			util.getRegGroup(res, "<meta property=\"og\\:title\" content=\"(.+?) - ニコニコチャンネル") :
            getRegGroup(res, "<meta property=\"og:site_name\" content=\"(.+?)\"")
            : getRegGroup(res, "<meta property=\"og\\:title\" content=\"(.+?)-ニコニコミュニティ\"");
        if (title == null)
            title = getRegGroup(res, "<meta property=\"og:title\" content=\"(.+?)さんのコミュニティ-ニコニコミュニティ\">");
...
//		System.Console.WriteLine(dt + " " + str);
        ...
        {
            debugWriteLine(e.Message + " " + e.StackTrace + " " + e.TargetSite + " " + e.Source);
        }
    }
    ...
    public static void showException(Exception eo, bool isMessageBox = true)
    {
        ...
        if (isMessageBox && isLogFile)
            if (frameCount > 50)
            {
                showMessageBoxCenterForm(null, "framecount stack", frameCount.ToString());
                return;
            }
#else
#endif
        ...
        debugWriteLine("0 message " + eo.Message + "\nsource " +
        ...
        if (_eo != null)
            debugWriteLine("1 message " + _eo.Message + "\nsource " +
        ...
        debugWriteLine("eo " + _eo);
        if (_eo != null)
            debugWriteLine("2 message " + _eo.Message + "\nsource " +
                           _eo.Source + "\nstacktrace " + _eo.StackTrace +
                           "\n targetsite " + _eo.TargetSite);

#if DEBUG
        if (isMessageBox && isLogFile)
            showMessageBoxCenterForm(null, "error", "error");
#else
#endif
    }
    ...
    public static Bitmap getSamune(string url)
    {
        ...
        try
        {
            debugWriteLine("samune url " + url);
            var pic = cl.DownloadData(url);

            using (var st = new MemoryStream(pic))
            {
                icon = Icon.FromHandle(new Bitmap(st).GetHicon());
            }
            //st.Close();
        }
        catch (Exception e)
