    ...
    public void rec()
    {
        ...
        while (true)
        {
            try
            {
                ...
                util.debugWriteLine("rlm.reclistdata.count " +
                                    _count); // + " reclist count " + rlm.form.recList.Rows.Count);
                for (var i = 0; i < _count; i++)
                {
                    if (rlm.rdg == null) return;
                    if (_count != rlm.form.getRecListCount()) break;
                    util.debugWriteLine("i " + i + " count " + _count);
                    var ri = rlm.recListData[i];
...
//			ri.state = "録画中";
    ...
    private void startRecProcess(RecInfo ri)
    {
        ...
        try
        {
            ...
            var isGetComment = ri.recComment == "映像＋コメント" || ri.recComment == "コメントのみ"
                ? " -IsgetComment=true"
                : " -IsgetComment=false";
    ...
    */
    ...
    {
        foreach (var ri in rlm.recListData)
            try
            {
                if (ri.state == "録画中")
                {
