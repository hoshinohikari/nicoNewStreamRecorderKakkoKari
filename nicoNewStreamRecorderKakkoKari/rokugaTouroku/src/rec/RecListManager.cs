...
public class RecListManager
{
    ...
    public bool add(string t)
    {
        ...
        else if (userId != null)
        {
            ...
            var r = util.showMessageBoxCenterForm(form, "ユーザーID" + userId + "のタイムシフトを登録しますか？ " + lvList.Count + "件",
        ...
        else if (chId != null)
        {
            ...
            var r = util.showMessageBoxCenterForm(form, "チャンネルID" + chId + "のタイムシフトを登録しますか？ " + lvList.Count + "件",
                "", MessageBoxButtons.YesNo);
            if (r == DialogResult.No) return false;
        }
        else if (coId != null)
        {
            ...
            var r = util.showMessageBoxCenterForm(form, "コミュニティID" + coId + "のタイムシフトを登録しますか？ " + lvList.Count + "件",
                "", MessageBoxButtons.YesNo);
            if (r == DialogResult.No) return false;
        }
        else
        ...
        foreach (var _lv in lvList)
        {
            ...
            try
            {
                if (bool.Parse(cfg.get("IsDuplicateConfirm")))
                {
                    ...
                    foreach (var d in recListData)
                        if (d.id == _lv)
                            delList.Add(d);

                    foreach (var _ri in delList)
                        if (util.showMessageBoxCenterForm(form,
                                _ri.id + "はリスト内に含まれています。既にある行を削除しますか？\n[" +
                                (string.IsNullOrEmpty(_ri.title) ? "タイトル未取得" : _ri.title) + "][" + _ri.state +
                                "][" + _ri.quality + "] [" + _ri.timeShift + "]", "確認", MessageBoxButtons.YesNo) ==
                            DialogResult.Yes)
                            form.deleteRow(_ri);
                }
            }
            ...
            var ri = new RecInfo(_lv, url, rdg, form.afterConvertModeList.Text, form.setTsConfig,
                form.setTimeshiftBtn.Text, form.qualityBtn.Text, form.qualityRank, form.recCommmentList.Text,
                form.isChaseChkBox.Checked, (AccountInfo)form.accountBtn.Tag);
    ...
    private List<string> getUserChannelStreamList(string id, string type)
    {
        try
        {
            ...
            for (var i = 0; i < (type == "user" ? 2 : 10); i++)
            {
                ...
                {
                    url = "https://api.cas.nicovideo.jp/v2/tanzakus/" + type + "/" + id +
                          "/content-groups/live/items?_offset=" + (10 + (i - 1) * 15) +
                          "&_limit=15&sort=liveRecent_asc";
                ...
                var r = util.sendRequest(url, h, null, "GET");
                if (r == null)
                {
                    util.showMessageBoxCenterForm(form,
                        (type == "user" ? "ユーザー" : "チャンネル") + id + "の放送情報が取得できませんでした");
                    return null;
                }
    ...
    private List<string> getCommunityStreamList(string id)
    {
        try
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                var url = "https://com.nicovideo.jp/api/v1/communities/" + _id + "/lives.json?limit=30&offset=" +
                          i * 30;
    ...
    private List<string> getCommunityJsonStreamLvList(string res)
    {
        try
        {
            ...
            if (id.Count != status.Count - 1 || id.Count != canView.Count || id.Count != finishedAt.Count / 2)
                return null;
            var ret = new List<string>();
