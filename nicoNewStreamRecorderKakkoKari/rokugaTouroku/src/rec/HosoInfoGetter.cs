...
public class HosoInfoGetter
{
    ...
    public bool get(string _url)
    {
        ...
        if (_dt != null)
        {
            ...
            _dt = util.getRegGroup(_dt, "(\\d{4}/\\d{1,2}/\\d{1,2})") + " " +
                  util.getRegGroup(_dt, "(\\d{4}:\\d{1,2}(:\\d{1,2})*)");
...
//				if (communityId == null) communityId = null;
    ...
    private bool setNicoLiveInfo(string res)
    {
        ...
        if (data == null) data = HttpUtility.HtmlDecode(res);
        type = util.getRegGroup(res, "\"content_type\":\"(.+?)\"");
        if (type == null) type = util.getRegGroup(res, "content_type = '(.+?)'");
        group = util.getRegGroup(data, "\"socialGroup\".+?\"name\".\"(.+?)\"");
        if (group == null)
            group = util.getRegGroup(res,
                "COMMUNITY INFO[\\s\\S]+target=\"_blank\"><span itemprop=\"name\">(.+?)</span>");
...
//					communityId = null;
            ...
            else
            {
                ...
                userId = isChannel
                    ...
                    : util.getRegGroup(data,
                        "supplier\":{\"name\".+?pageUrl\":\"http[s]*://www.nicovideo.jp/user/(\\d+?)\"");
        ...
        else
        {
            ...
            if (!isChannel)
                userId = util.getRegGroup(res,
                    "<a href=\"https*://www.nicovideo.jp/user/(\\d+)\" target=\"_blank\">");
            else
            ...
            if (communityId == null)
                communityId = util.getRegGroup(res, "<a href=\"http.+?/community/(co\\d+)\" target=\"_blank\">");

            if (type == "official")
                description = util.getRegGroup(res, "description:.*?['\"]([\\s\\S]*?[^\\\\])'");
            else
                description = util.getRegGroup(res,
                    "<div class=\"gate_description_area\">([\\s\\S]*?)(<div|</div>)");
        }
