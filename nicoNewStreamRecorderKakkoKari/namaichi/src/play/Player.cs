...
public class Player
{
    ...
    private Process commentProcess;
    private defaultFFplayController ctrl;
    private bool isDefaultCommentPlayer;
    private bool isDefaultPlayer;
    public bool isReconnect;

    private bool isRecording;
    private bool isUseCommentViewer;

    private bool isUsePlayer;
                        ...
                        while (true)
                        {
                            ...
                            if ((isChangeUrlOk && form.rec.hlsUrl != null
                                               && (form.rec.hlsUrl.StartsWith("http") ||
                                                   form.rec.hlsUrl.StartsWith("-vr"))) || isReconnect)
                            {
                                isReconnect = false;
        ...
        if (isDefaultPlayer)
        {
            try
            {
                ...
                if (form.rec.hlsUrl.StartsWith("http"))
                    ...
                    Task.Run(() => playCommandStd("ffplay.exe",
                        form.rec.hlsUrl + " -autoexit -volume " + volume + " " + exArgs));
            ...
            {
                util.debugWriteLine("sendPlayCommand exception " + e.Message + e.Source + e.StackTrace +
        ...
        if (isStart)
            Task.Run(() =>
            {
                if (isDefaultCommentPlayer)
                {
                    ...
                    while (isRecording) Thread.Sleep(300);
...
//			process.StartInfo.RedirectStandardError = true;
        ...
        catch (Exception ee)
        {
            ...
            form.addLogText("デフォルトプレイヤー: " + isDefaultPlayer);
    ...
    private void playCommandStd(string exe, string args)
    {
        try
        {
            ...
            var rtmpPath = bool.Parse(config.get("IsDefaultRtmpPath")) ? "rtmpdump.exe" : config.get("rtmpPath");
...
//			process.StartInfo.CreateNoWindow = true;
    ...
    {
        form.Invoke((MethodInvoker)delegate { form.playerBtn.Text = s; });
    }
    ...
    private void ctrlFormClose()
    {
        ...
        try
        {
            ctrl.Invoke((MethodInvoker)delegate
            {
                ...
                {
                    util.debugWriteLine(
                        "ctrl close exception " + e.Message + e.Source + e.StackTrace + e.TargetSite);
                }
            });
        }
        catch (Exception ee)
    ...
    private void defaultCommentFormClose()
    {
        ...
        try
        {
            commentForm.Invoke((MethodInvoker)delegate
            {
                ...
                {
                    util.debugWriteLine("comment form close exception " + e.Message + e.Source + e.StackTrace +
                                        e.TargetSite);
                }
            });
        }
        ...
        {
            util.debugWriteLine("comment form close2 exception " + ee.Message + ee.Source + ee.StackTrace +
                                ee.TargetSite);
        }
    }
    ...
    private bool getHlsUrl()
    {
        if (form.rec.rfu == null)
        {
            ...
            form.rec.rec(true);
            form.Invoke((MethodInvoker)delegate { form.recBtn.Enabled = false; });
            if (form.rec.rfu == null) return false;
            while (form.rec.rfu != null)
            {
                if (form.rec.hlsUrl == "end") return false;
                if (form.rec.hlsUrl != null) return true;
                Thread.Sleep(300);
            }
