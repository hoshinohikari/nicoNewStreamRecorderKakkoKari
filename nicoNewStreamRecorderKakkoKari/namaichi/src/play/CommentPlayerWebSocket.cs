...
{
    private readonly commentForm cf;
    private readonly IRecorderProcess wsr;
    private DeflateDecoder deflateDecoder = new();
...
//			msThread = wsr.msReq;
        ...
        try
        {
            ...
            wsc = new WebSocket(msUri, "", null, header, util.userAgent, "", WebSocketVersion.Rfc6455, null,
        ...
        {
            util.debugWriteLine("wsc player connect exception " + ee.Message + ee.Source + ee.StackTrace +
                                ee.TargetSite);
            return;
        }
...
//			string msg = (wsr.isJikken) ? deflateDecoder.decode(e.Data) : e.Message;
        ...
        foreach (var chatinfo in chatInfoList)
        {
            ...
            util.debugWriteLine("player xml " + chatXml);
