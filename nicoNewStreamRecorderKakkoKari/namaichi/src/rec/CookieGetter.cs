...
public class CookieGetter
{
    ...
    private static readonly Uri TargetUrl4 = new("https://watch.live.nicovideo.jp/api/");
    private static readonly Uri TargetUrl5 = new("https://www.nicovideo.jp/");
    public static bool isLoginCheck = false;
    private readonly config.Config cfg;

    //private bool isSub;
    private readonly bool isRtmp = false;
    public string id;
    ...
    public async Task<CookieContainer[]> getHtml5RecordCookie(string url, bool isSub = false)
    {
        CookieContainer cc;
        ...
        bool isSub, string url)
    {
        if (cfg.argAi != null)
        {
            ...
            {
                reason = null;
                var cc = await getBrowserCookie(isSub, cfg.argAi.si).ConfigureAwait(false);
                log += cc == null
                    ? "引数で指定されたブラウザからユーザーセッションを取得できませんでした。ログインに使うブラウザの設定をご確認ください。他のブラウザやアカウントログインを試したり、ブラウザ上で一度ログインし直した後にもう一度ツール側で設定すると上手くいくかもしれません。"
...
//				util.debugWriteLine(userSessionCC.GetCookieHeader(TargetUrl));
            ...
            if (isHtml5Login(userSessionCC, url)) return userSessionCC;
        }
        ...
        if (browserNum == "2")
        {
            ...
            log += cc == null
                ? "ブラウザからユーザーセッションを取得できませんでした。ログインに使うブラウザの設定をご確認ください。他のブラウザやアカウントログインを試したり、ブラウザ上で一度ログインし直した後にもう一度ツール側で設定すると上手くいくかもしれません。"
                : "ブラウザからユーザーセッションを取得しました。";
            if (cc != null)
            {
                util.debugWriteLine("browser ishtml5login");
...
//			cc.Add(TargetUrl, c);
    ...
    private async Task<CookieContainer> getBrowserCookie(bool isSub, CookieSourceInfo si = null)
    {
    ...
    private bool isHtml5Login(CookieContainer cc, string url)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            try
            {
                ...
                var _url = isRtmp
                    ? "https://live.nicovideo.jp/api/getplayerstatus/" + util.getRegGroup(url, "(lv\\d+)")
                    : url;
            ...
            if (pageSource.IndexOf("content=\"https://jk.nicovideo.jp/\"") != -1) return true;
