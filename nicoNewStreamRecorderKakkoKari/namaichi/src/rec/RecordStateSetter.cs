...
public class RecordStateSetter
{
    ...
    private readonly bool isChannelPlus;

    private readonly bool isDescriptionTag;
    private readonly bool isJikken;

    //private bool isChase = false;
    private readonly bool isReservation;
    private readonly bool isRtmpOnlyPage;
    ...
    public RecordStateSetter(MainForm form, bool isJikken, bool isPlayOnlyMode, bool isRtmpOnlyPage,
        bool isReservation, bool isChannelPlus = false)
    {
        this.form = form;
        //this.rm = rm;
        //this.rfu = rfu;
        ...
        isDescriptionTag = bool.Parse(form.rec.cfg.get("IsDescriptionTag"));
...
//			var a = await setInfo(data, form, type, recFolderFileInfo).ConfigureAwait(false);
    ...
    private void setInfo(string data, MainForm form, string type, string[] recFolderFileInfo)
    {
        ...
        {
            hostUrl = type == "community" || type == "user"
                ? util.getRegGroup(data, "supplier\":{\"name\".\".+?\",\"pageUrl\":\"(.+?)\"")
                : null;
        ...
        {
            hostUrl = type == "community" || type == "user"
                ? util.getRegGroup(data, "broadcaster\":{.+?\"pageUrl\":\"(.+?)\"")
                : null;
...
//			util.debugWriteLine(a);
    ...
    public void writeEndTime(CookieContainer cc, DateTime endTime)
    {
        ...
        {
            form.addLogText("放送終了時間を番組情報テキストへ書き込みに問題が発生しました。 " + e.Message + e.Source + e.StackTrace +
                            e.TargetSite);
            form.addLogText(fName);
        }
    }
    ...
    private void setStatistics(string data)
    {
        try
        {
            if (!isChannelPlus)
            {
                var _watchCount = util.getRegGroup(data,
                    "\"statistics\":\\{\"watchCount\":(\\d+),\"commentCount\":\\d*\\}");
                var _commentCount = util.getRegGroup(data,
                    "\"statistics\":\\{\"watchCount\":\\d*,\"commentCount\":(\\d+)\\}");
