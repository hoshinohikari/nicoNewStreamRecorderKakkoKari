...
public class Record
{
    public static bool isWriteCancel;

    private readonly string anotherEngineCommand = "";
    ...
    private readonly Curl getM3u8Curl;
    private readonly Curl getTsCurl;
    private readonly List<numTaskInfo> gotTsList = new();
    private readonly bool isPlayOnlyMode;

    //private bool isTimeShift = false;
    //public TimeShiftConfig tsConfig = null;
    private readonly List<numTaskInfo> newGetTsTaskList = new();
    private readonly List<string> recordedNo = new();
    private readonly object recordLock = new();

    private readonly string referer;

    //private bool isFFmpeg;
    private readonly RecordFromUrl rfu;
    private readonly RecordingManager rm;
    private readonly List<string> segM3u8List = new();

    private readonly int segmentSaveType;
    private readonly string ua = null; //"Lavf/56.36.100";
    private readonly IRecorderProcess wr;
...
//		private RealTimeFFmpeg realtimeFFmpeg = null;
    ...
    private numTaskInfo lastWriteFmp4Nti;
...
//		private DateTime lastTsPlayListTime = DateTime.MinValue;
    ...
    private Task m3u8GetterTask;
    public string recFolderFile;
    private long recordedBytes;

    private double recordedSecond;
    ...
    */
        ...
        WebSocket ws, RecordInfo ri)
    {
        this.rm = rm;
        this.rfu = rfu;
        recFolderFile = ri.recFolderFile[1];
        hlsMasterUrl = hlsUrl;
        this.container = container;
...
//			rm.form.setPlayerBtnEnable(false);
        ...
        if (rm.cfg.get("fileNameType") == "10" &&
            (recFolderFile.IndexOf("{w}") > -1 || recFolderFile.IndexOf("{c}") > -1))
            renameStatistics();
        ...
        if (engineMode == "0" && !isPlayOnlyMode)
        {
            ...
            if (gotTsList.Count > 10) displayWriteRemainGotTsData();
            waitForRecording();

            if (rfu.subGotNumTaskInfo != null)
                addDebugBuf("rfu.subGotNumTaskInfo.count " + rfu.subGotNumTaskInfo.Count);
            var isManyNti = rfu.subGotNumTaskInfo == null || rfu.subGotNumTaskInfo.Count == 0
                ? false
                : rfu.subGotNumTaskInfo[rfu.subGotNumTaskInfo.Count - 1].dt - rfu.subGotNumTaskInfo[0].dt >
                  TimeSpan.FromSeconds(25);
            if (isManyNti)
                addDebugBuf("isManyNti subgot [0].dt + " + rfu.subGotNumTaskInfo[0].dt + " [count - 1] " +
                            rfu.subGotNumTaskInfo[rfu.subGotNumTaskInfo.Count - 1].dt);
//				isManyNti = true;
            if (rfu.subGotNumTaskInfo != null && isManyNti)
            {
                ...
                if (dsp == null)
                {
                    dsp = new DropSegmentProcess(lastWroteSegmentDt, lastSegmentNo, this, ri.recFolderFile[2], rfu,
                        rm, ri);
                    dsp.writeRemaining();
                    dsp = null;
                }
...
//					     	int.Parse(rm.cfg.get("afterConvertMode")) != 1)) {
    ...
    private void startM3u8Getter()
    {
        ...
        while (rm.rfu == rfu && isRetry)
            try
            {
                ...
                {
                    reconnectingCount++;
                    if (reconnectingCount > 20) isReConnecting = false;
                    Thread.Sleep(500);
                    continue;
                }
                ...
                var _targetDuration = util.getRegGroup(res, "#EXT-X-TARGETDURATION:(\\d+(\\.\\d+)*(e\\d+)*)", 1,
                    rm.regGetter.getExtXTargetDuration());
                if (_targetDuration != null) targetDuration = double.Parse(_targetDuration, NumberStyles.Float);
                var intervalTime = (int)(targetDuration * 1000);
                var elapsedTime = (int)(DateTime.Now - lastAccessPlaylistTime).TotalMilliseconds;
            ...
            catch (Exception e)
            {
                ...
                rm.form.addLogText("プレイリストの取得中に何らかの問題が発生しました " + e.Message + e.Source + e.StackTrace +
                                   e.TargetSite);
            }
    ...
    private void startTsGetter()
    {
        while (true)
                ...
                foreach (var _s in new List<string>(segM3u8List))
                {
                    ...
                    foreach (var s in _s.Split('\n'))
                    {
                        ...
                        var _targetDuration = util.getRegGroup(s, "^#EXT-X-TARGETDURATION:(\\d+(\\.\\d+)*(e\\d+)*)",
                            1, rm.regGetter.get_ExtXTargetDuration());
                        ...
                        var _allDuration = util.getRegGroup(s, "^#STREAM-DURATION:(.+)", 1,
                            rm.regGetter.getStreamDuration());
                        if (_allDuration != null) allDuration = double.Parse(_allDuration, NumberStyles.Float);
                        ...
                        if (no + baseNo > lastSegmentNo && !isInList && no + baseNo > gotTsMaxNo)
                        {
                            ...
                            fileName = no + baseNo + "_" + startTimeStr + ext;
            ...
            {
                addDebugBuf(e.Message + e.Source + e.StackTrace + e.TargetSite);
                rm.form.addLogText("動画ファイルの取得中に何らかの問題が発生しました " + e.Message + e.Source + e.StackTrace +
                                   e.TargetSite);
            }
    ...
    private void startTsWriter()
    {
        while (rfu == rm.rfu)
            try
            {
                ...
                {
                    addDebugBuf("startTsWriter end isTsGetterEnd " + isTsGetterEnd + " gotTsList.count " +
                                gotTsList.Count);
                    break;
                }
                ...
                foreach (var s in new List<numTaskInfo>(gotTsList))
                {
                    ...
                    addDebugBuf("s.no " + s.no + " s.originNo " + s.originNo + " lastsegmentno " + lastSegmentNo +
                                " " + baseNo);
                    ...
                    if (ret)
                    {
                        ...
                        if (isNuke)
                        {
                            ...
                            addDebugBuf("nuke ari s.no " + s.no + " lastsegmentno " + lastSegmentNo +
                                        " s.originNo " + s.originNo);
            ...
            catch (Exception e)
            {
                addDebugBuf(e.Message + e.Source + e.StackTrace + e.TargetSite);
                rm.form.addLogText(
                    "動画データの書き込み中に何らかの問題が発生しました " + e.Message + e.Source + e.StackTrace + e.TargetSite);
            }
...
//			var wc = new WebHeaderCollection();
        ...
        addDebugBuf("addNewTsTaskList segm3u get len 1000 " +
                    (res == null ? "" : res.Substring(0, res.Length > 1000 ? 1000 : res.Length)));
//			util.debugWriteLine("m3u8 " + res);
        ...
        //shuusei? 
        var min = res == null ? -1 : int.Parse(util.getRegGroup(res, "(\\d+)" + ext, 1, rm.regGetter.getTs()));
...
//				if (!isReConnecting) 
        ...
        var isTimeShiftPlaylist = res.IndexOf("#STREAM-DURATION") > -1;
        if (!ri.si.isTimeShift && isTimeShiftPlaylist) return -1;
        ...
        lock (recordLock)
        {
            ...
            foreach (var s in res.Split('\n'))
            {
                ...
                util.debugWriteLine("ri.timeShiftConfig type time " + ri.timeShiftConfig.timeType + " " +
                                    ri.timeShiftConfig.timeSeconds);
                var startTime = baseTime + secondSum - second;
                var startTimeStr = util.getSecondsToStr(startTime);
                ...
                if (ri.si.isTimeShift && ri.timeShiftConfig.endTimeSeconds > 0 &&
                    startTime >= ri.timeShiftConfig.endTimeSeconds)
                {
                    addDebugBuf("timeshift ri.timeShiftConfig.endtime " + ri.timeShiftConfig.endTimeSeconds +
                                " now starttime " + startTime + " ri.timeShiftConfig.timeseconds " +
                                ri.timeShiftConfig.timeSeconds);
                    _isRetry = false;
                    _isEndProgram = true;
                    continue;
        ...
        try
        {
            lock (newGetTsTaskList)
            {
                for (var i = 0; i < newGetTsTaskList.Count; i++)
                    ...
                    {
                        if (tsBytes == null || (!ri.si.isChannelPlus && lastSegmentNo > 10000 &&
                                                newGetTsTaskList[i].no - lastSegmentNo > 5500))
                        {
                            ...
                            if (!isReConnecting)
                            {
                                ...
                                //debug
                                util.debugWriteLine("get bytes " + tsBytes + " / lastsegno " + lastSegmentNo +
                                                    "/ no " + (newGetTsTaskList != null &&
                                                               newGetTsTaskList.Count > 0
                                                        ? newGetTsTaskList[0].no
                                                        : -10));
                ...
                {
                    addDebugBuf(
                        "write file " + i + " url " + newGetTsTaskList[i].no + " lastsegNo " + lastSegmentNo);
                    if (newGetTsTaskList[i].res == null) break;

                    if (newGetTsTaskList[i].no <= lastSegmentNo) continue;
                    bool ret;
                    if (isPlayOnlyMode)
                    {
                        ret = true;
...
//							if (fName == 
                        ...
                        util.debugWriteLine("aaaaa " + ri.timeShiftConfig.endTimeSeconds + " " + startTime + " " +
                                            newGetTsTaskList[i].startSecond + " " + nti.second + " a " +
                                            (startTime + nti.second) + " " +
                                            (startTime + nti.second >= ri.timeShiftConfig.endTimeSeconds) + " " +
                                            newGetTsTaskList.Count + " " + i);
                        if (ri.si.isTimeShift && ri.timeShiftConfig.endTimeSeconds > 0 &&
                            newGetTsTaskList[i].startSecond + nti.second >= ri.timeShiftConfig.endTimeSeconds)
                        {
                            addDebugBuf("getTsTask timeshift ri.timeShiftConfig.endtime " +
                                        ri.timeShiftConfig.endTimeSeconds + " now starttime " + startTime +
                                        " ri.timeShiftConfig.timeseconds " + ri.timeShiftConfig.timeSeconds);
                            isRetry = false;
                            isEndProgram = true;
    ...
    private void setRecordState()
    {
        ...
        //	_kb = " " + _kb;
        //ret += "size=" + _kb + "kB\n";
        var size = bytes > 1000000 ? (bytes / 1000000.0).ToString("n3") + "mB" : (int)(bytes / 1000) + "kB";
        ret += "size=   " + size + "\n";
    ...
    public void reSetHlsUrl(string url, string quality, WebSocket _ws, bool isChaseRealing)
    {
        ...
        if (!ri.si.isChannelPlus)
        {
                ...
                !(ri.isChase && lateTime < 15))
            {
                ...
                {
                    if (isChaseRealing)
                        start = lastRecordedSeconds + 20;
                    else start = lastRecordedSeconds - 10 < 0 ? 0 : lastRecordedSeconds - 10;
                }

                hlsMasterUrl = hlsMasterUrl + "&start=" + start;

                wr.tsHlsRequestTime = DateTime.Now;
                wr.tsStartTime = TimeSpan.FromSeconds(start);
            }
...
//				   	int.Parse(rm.cfg.get("afterConvertMode")) > 1) {
    ...
    private void ffmpegRenketuAfter()
    {
        ...
        var args = "-i \\\\.\\pipe\\" + pipeName + " -c copy \"" + recFolderFile + "/" + fName + ext + "\"";
    ...
    private string streamRenketuAfter()
    {
        ...
        catch (PathTooLongException)
        {
            try
            {
                ...
                using (w = new FileStream(recFolderFile + "/" + ri.si.lvid + ext, FileMode.Append,
                           FileAccess.Write))
                {
                    _streamRenketuAfterWrite(w);
                    return w.Name;
    ...
    private void _streamRenketuAfterWrite(FileStream w)
    {
        foreach (var s in recordedNo)
        {
            ...
            {
                addDebugBuf("renketu after write exception " + s + " " + e.Message + e.Source + e.StackTrace +
                            e.TargetSite);
            }
        }
    }
    ...
    private void timeShiftOnTimeRecord()
    {
        ...
        */
        if (hlsSegM3uUrl == null)
        {
            ...
            if (!ri.isRealtimeChase) baseMasterUrl += "&start=" + start;
        ...
        while (rm.rfu == rfu && isRetry)
        {
            ...
            if (engineMode == "0" || engineMode == "3")
            {
                ...
                var elapsedTime = (int)(DateTime.Now - lastAccessPlaylistTime).TotalMilliseconds;
                util.debugWriteLine("wait time intervalTime " + intervalTime + " elapsedTime " + elapsedTime +
                                    " recordedSeconds " + recordedSecond);
                Thread.Sleep(elapsedTime > intervalTime ? 0 : intervalTime - elapsedTime);
                lastAccessPlaylistTime = DateTime.Now;
            }
            else
            {
                ...
                var startPlayList = !ri.si.isChannelPlus
                    ? util.getPageSource(hlsSegM3uUrl, null, referer, false, 2000, ua)
                    : getM3u8Curl.getStr(hlsSegM3uUrl, Curl.getDefaultHeaders(),
                        CurlHttpVersion.CURL_HTTP_VERSION_3);
                ...
                if (isEndProgram || (isAnotherEngineTimeShiftEnd(recStartTime, hlsSegM3uUrl, startPlayList) &&
                                     !ri.isRealtimeChase))
                {
                    isEndProgram = true;
                    break;
...
//			rm.form.setPlayerBtnEnable(false);
        ...
        if (engineMode == "0" && !isPlayOnlyMode)
        {
            ...
            if (isEndProgram && segmentSaveType == 0 && !isTsEndTimeEnd) renameWithoutTime(recFolderFile);
            if (segmentSaveType == 1 &&
                rm.cfg.get("IsRenketuAfter") == "true")
            {
                //				     || int.Parse(rm.cfg.get("afterConvertMode")) > 1)
...
//				    	    int.Parse(rm.cfg.get("afterConvertMode")) > 1) {
        ...
        if (isEndProgram && !wr.isHokan) rm.form.addLogText("録画を完了しました");
        isEnd = true;
    }
            ...
            try
            {
                ...
                lock (gotTsList)
                {
                    for (var i = 0; i < (gotTsList.Count < 5 ? gotTsList.Count : 5); i++)
                    {
                        ...
                        util.debugWriteLine("timeshift writer write " + r + " " + gotTsList[i].no + " " +
                                            gotTsList.Count + " " + i + " " + rm.hlsUrl);
                        if (isWriteCancel) return;
                        if (r)
                        {
    ...
    private double getBaseTimeFromPlaylist(string res)
    {
        ...
        var mediaSequenceNum =
            util.getRegGroup(res, "#EXT-X-MEDIA-SEQUENCE\\:(.+)", 1, rm.regGetter.getExtXMediaSequence());
    ...
    private void startDebugWriter()
    {
        ...
        util.debugWriteLine("start debug writer end rm.rfu == rfu " + (rm.rfu == rfu) + " isend " + isEnd +
                            " debugWriteBuf.count " + debugWriteBuf.Count);
    }
    ...
    private string getAddedNearList(string res)
    {
        ...
        if (lastSegmentNo == 0 && false) return res;
        var maxNo = 0;
        var maxLine = "";
        var minNo = 0;
        foreach (var l in res.Split('\n'))
        ...
        addDebugBuf("minNo " + minNo + " lastSegmentNo " + lastSegmentNo + "lastGetPlayListMaxNo " +
                    lastGetPlayListMaxNo + " base no " + baseNo);
        //if (minNo > lastGetPlayListMaxNo) {
        if (minNo + baseNo > gotTsMaxNo)
        {
            addDebugBuf("nuke? minNo " + minNo + " lastSegmentNo " + lastSegmentNo + "lastGetPlayListMaxNo " +
                        lastGetPlayListMaxNo + " gottsmaxno " + gotTsMaxNo);
            if (res == null)
            {
            ...
            var ins = "";
            var sakanoboriNo = 3;
            var startNo = minNo + baseNo - gotTsMaxNo > sakanoboriNo
                ? minNo - sakanoboriNo
                : gotTsMaxNo - baseNo + 1;
            for (var i = startNo; i < minNo; i++)
            {
                if (i < 0) continue;
                ins += "#EXTINF:" + inf + ",\n";
                ins += maxLine.Replace(maxNo + ext, i + ext) + "\n";
            }
    ...
    */
    ...
    private bool isEndTimeshift(double streamDuration, string res, double second)
    {
        if (ri.si.isChannelPlus)
        {
            ...
            if (tsNumList.Count > 0)
            {
                var last = int.Parse(tsNumList[tsNumList.Count - 1].Groups[1].Value +
                                     tsNumList[tsNumList.Count - 1].Groups[2].Value);
        ...
        addDebugBuf("isendTimeshift streamDuration " + streamDuration + " second " + second + " lastSegmentNo " +
                    lastSegmentNo + " " + lastWroteFileSecond + " ret " + ret);
    ...
    private bool isAnotherEngineTimeShiftEnd(DateTime recStartTime, string hlsSegM3uUrl, string startPlayList)
    {
        ...
        double streamDuration = -1;
        var _streamDuration = startPlayList == null
            ? null
            : util.getRegGroup(startPlayList, "#STREAM-DURATION:(.+)", 1, rm.regGetter.getStreamDuration());
        if (_streamDuration == null) return false;
        streamDuration = double.Parse(_streamDuration, NumberStyles.Float);

        var diffTime = DateTime.Now - recStartTime;
    ...
    private void dropSegmentProcess(numTaskInfo s, DateTime _lastWroteSegmentDt, int _lastSegmentNo)
    {
        if (dsp == null)
        {
            dsp = new DropSegmentProcess(_lastWroteSegmentDt, _lastSegmentNo, this, ri.recFolderFile[2], rfu, rm,
                ri);
...
//					addDebugBuf("delete old subts i " + i + " c " + c + " count " + rfu.subGotNumTaskInfo.Count);
        ...
        catch (Exception e)
        {
            ...
            addDebugBuf(
                "lastFileSecond " + lastFileSecond + " lastWroteSegmentDt " + lastWroteSegmentDt + " i " + i);
            addDebugBuf("subgotNumTaskInfo.count " + rfu.subGotNumTaskInfo.Count + " [0] " +
                        (rfu.subGotNumTaskInfo.Count > 0 ? rfu.subGotNumTaskInfo[0] : null));
#if DEBUG
...
//			});
    ...
    private void renameWithoutTime(string name)
    {
        ...
        try
        {
            for (var i = int.Parse(num); i < 1000; i++)
            {
    ...
    private void renameStatistics()
    {
        try
        {
            ...
            if (File.Exists(recFolderFile + ext))
            {
                var newName = recFolderFile.Replace("{w}", wr.visitCount.Replace("-", ""))
                    .Replace("{c}", wr.commentCount.Replace("-", ""));
                File.Move(recFolderFile + ext, newName + ext);
                recFolderFile = newName;
            }
    ...
    */
    ...
    private int getChangedTfdtMp4(byte[] b, int baseTfdt, out byte[] newTfdtB)
    {
        ...
        try
        {
            ...
            using (var ms = new MemoryStream())
            {
                while (pos < b.Length)
                {
                    var size = BitConverter.ToInt32(new[] { b[pos + 3], b[pos + 2], b[pos + 1], b[pos + 0] }, 0);
                    if (size == 0)
                    {
                    ...
                    if (type == "moof")
                    {
                        ...
                        while (_pos < pos + size)
                        {
                            var _size = BitConverter.ToInt32(
                                new[] { b[_pos + 3], b[_pos + 2], b[_pos + 1], b[_pos + 0] }, 0);
                            if (_size == 0)
                            {
                                util.debugWriteLine("atom parse error size 0");
                                rm.form.addLogText("atom parse error size 0");
                            ...
                            if (_type == "traf")
                            {
                                ...
                                {
                                    var __size = BitConverter.ToInt32(
                                        new[] { b[__pos + 3], b[__pos + 2], b[__pos + 1], b[__pos + 0] }, 0);
                                    if (__size == 0)
                                    {
                                        util.debugWriteLine("atom parse error size 0");
                                        rm.form.addLogText("atom parse error size 0");
                                    ...
                                    if (__type == "tfdt")
                                    {
                                        var ___size = BitConverter.ToInt32(
                                            new[] { b[__pos + 3], b[__pos + 2], b[__pos + 1], b[__pos + 0] }, 0);
                                        if (___size == 0)
                                        {
                                            util.debugWriteLine("atom parse error size 0");
                                        ...
                                        var tfdt = BitConverter.ToInt32(
                                            new[]
                                            {
                                                b[__pos + ___size - 1], b[__pos + ___size - 2],
                                                b[__pos + ___size - 3], b[__pos + ___size - 4]
                                            }, 0);
                                        ...
                                        if (isLog)
                                            util.debugWriteLine("      new tfdt " + (tfdt - baseTfdt) + " " +
                                                                newTfdt);

                                        for (var i = 1; i < 5; i++)
                                            b[__pos + ___size - i] = i > newTfdt.Length ? (byte)0 : newTfdt[i - 1];
                                        tfdt = BitConverter.ToInt32(
                                            new[]
                                            {
                                                b[__pos + ___size - 1], b[__pos + ___size - 2],
                                                b[__pos + ___size - 3], b[__pos + ___size - 4]
    ...
    private void writeComplementMp4File(numTaskInfo nti)
    {
    ...
    private int getChannelPlusTimeShiftTotalTime(string res)
    {
...
internal class NtiGetter
{
    private readonly Curl curl;
    private readonly numTaskInfo nti;
    private readonly RecordInfo ri;
