...
//		private CookieContainer[] container;
    ...
    private bool isJikken;
    public bool isPlayOnlyMode;

    public bool isRtmpMain;
...
//		private JikkenRecorder jr;
    ...
    public int rec(string url, string lvid)
    {
        ...
        tsRecNumArr = this.lvid == lvid
            ? null
            : Array.ConvertAll(util.getRegGroup(lvid, ",(.+)").Split(','), i => { return int.Parse(i); });

        var mainT = Task.Run(() =>
        {
            return lvid.StartsWith("lv")
                ? _rec(this.url)
                : new ChannelPlusRecorder(url, rm.form, rm, lvid, this).run();
        });
    ...
    private int _rec(string url)
    {
        ...
        var pageType = getPageType(url, true, ref jr, out cc);
        ...
        while (this == rm.rfu)
        {
            ...
            if (pageType == 4)
            {
                ...
                if (bool.Parse(rm.cfg.get("IsmessageBox")) && util.isShowWindow)
                {
                    var ret = rm.form.formAction(() =>
                        util.showMessageBoxCenterForm(rm.form,
                            "コミュニティに入る必要があります：\nrequire_community_member/" + lvid), false);
                    if (!ret) return 2;
                }
            ...
            if (pageType == 9)
            {
                ...
                var isYoyakuRes = DialogResult.None;
                ...
                if (reserveMessage != "ダイアログで確認")
                    ...
                    rm.form.formAction(
                        () =>
                        {
                            isYoyakuRes = util.showMessageBoxCenterForm(rm.form, "この番組の視聴には予約が必要です。予約しますか？", "",
                ...
                if (reserveRet == "予約リストが一杯です。")
                    ...
                    rm.form.formAction(() =>
                    {
                        var isOpenMypageRes = util.showMessageBoxCenterForm(rm.form, "予約リストが一杯です。マイページを開きますか？", "",
                            MessageBoxButtons.YesNo);
                        if (isOpenMypageRes == DialogResult.Yes)
                            Process.Start("https://live.nicovideo.jp/my");
                    }, false);
                return 2;
            }
            ...
            try
            {
                ...
                if (cgret == null || cgret.Result[0] == null)
                {
                    ...
                    Thread.Sleep(
                        ...
                        cg.pageSource.IndexOf("ただいまメンテナンス中です。") > -1
                            ? 300000
                            : 10000);
                    continue;
                }
                ...
                if (isRtmpMain)
                {
                    ...
                    if (res.IndexOf("player_type = null") > -1)
                    {
                        ...
                        if (res.IndexOf("\"Nicolive.WatchingReservation.confirm") > -1) return 10;
                        return 2;
                    }

                    if (res.IndexOf("player_type = 'flash'") > -1 &&
                        res.IndexOf("%3Cgetplayerstatus%20") > -1)
                    {
                        var _res = util.getRegGroup(res, "(%3Cgetplayerstatus%20.+?%3C%2Fgetplayerstatus%3E)");
                        _res = WebUtility.UrlDecode(_res);
...
#if DEBUG
                            rm.form.addLogText("(" + waitSecond + "秒)now" + DateTime.Now + " o" + opentime + " s" +
                                               serverTime);
#endif
...
//					if (isRtmp) pagetype = getRtmpPageType(res, isSub, out rr, cc);
    ...
    */
    private void setDisplay(string res)
    {
        try
        {
            ...
            Task.Run(() => { rss.set(si.data, si.recFolderFileInfo, res); });
        }
        catch (Exception e)
        {
            util.debugWriteLine(e.Message + e.Source + e.StackTrace + e.TargetSite);
