...
public class XmlCommentGetter_ontime
{
    ...
    private readonly string isGetCommentXml;
    private readonly bool isRetry = true;
    private readonly bool isRtmp;
    private readonly bool isTimeShift;
    private readonly string lvid;
    private readonly long openTime;
    private readonly string recFolderFile;
    private readonly RecordFromUrl rfu;
    private readonly RecordingManager rm;
    private readonly IRecorderProcess rp;
        ...
        long openTime, long _openTime, long serverTime)
    {
        ...
        isGetCommentXml = rm.cfg.get("IsgetcommentXml");
...
//			rm.form.addComment("RTMPモードでは画面にコメント・来場者数・コメント数が表示されません");
        ...
        while (rm.rfu == rfu && isRetry)
        {
            ...
            var request =
                Encoding.UTF8.GetBytes("<thread thread=\"" + thread +
                                       "\" version=\"20061206\" res_from=\"-5\" /> \0");
...
//			chatinfo.getFormatXml(serverTime);
        ...
        util.debugWriteLine("xml " + chatXml);
