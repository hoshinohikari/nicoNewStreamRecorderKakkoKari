...
public class OutputTimeShiftTsUrlList
{
    ...
    public void write(string playList, string _path, string baseUrl, TimeShiftConfig tsConfig)
    {
        ...
        var duration = double.Parse(_duration, NumberStyles.Float);
        ...
        if (startNum >= tsConfig.timeSeconds * 1000)
            ret += temp.Replace(tempNum + ext, startNum + ext);
    ...
    private void writeM3u8List(string path, string buf)
    {
        using (var w = new StreamWriter(path, false))
        {
            ...
            w.WriteLine("#EXT-X-TARGETDURATION:" + (tsConfig.m3u8UpdateSeconds + 3));
            w.Flush();
            var isOpened = false;
            foreach (var b in buf.Split('\n'))
            {
                w.WriteLine("#EXTINF:" + tsConfig.m3u8UpdateSeconds + ",");
                w.WriteLine(b);
                w.Flush();
                ...
                if (tsConfig.isOpenUrlList && !isOpened)
                {
                    ...
                    if (openRes != "ok") rm.form.addLogText(openRes);
                    isOpened = true;
                }
