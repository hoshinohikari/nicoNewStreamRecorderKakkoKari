...
public class Html5Recorder
{
    ...
    private int html5Record(string res, bool isRtmp, int pageType, string url, string lvid, bool isTimeShift)
    {
        ...
        while (rm.rfu == rfu)
        {
            ...
            //display set
            var rss = new RecordStateSetter(rm.form, false, rfu.isPlayOnlyMode, si.isRtmpOnlyPage,
                si.isReservation);
            ...
            try
            {
                ...
                if (rm.cfg.get("fileNameType") == "10" && (ri.recFolderFile[1].IndexOf("{w}") > -1 ||
...
//			string res = getPageSource(url);
    ...
    */
    ...
    private int processFromPageType(int pageType)
    {
        ...
        if (pageType == 0 || pageType == 7) return 0;
                    ...
                    rm.form.formAction(
                        () =>
                        {
                            util.showMessageBoxCenterForm(rm.form,
                                "コミュニティに入る必要があります：\nrequire_community_member/" + ri.si.lvid);
                        }, false);
                }
                catch (Exception e)
                {
            ...
            if (bool.Parse(rm.cfg.get("IsfailExit")) && util.isShowWindow)
            {
                ...
                try
                {
                    rm.form.formAction(() =>
                    {
                        ...
                        {
                            util.debugWriteLine(
                                e.Message + " " + e.StackTrace + " " + e.Source + " " + e.TargetSite);
                        }
                    });
                }
                catch (Exception e)
