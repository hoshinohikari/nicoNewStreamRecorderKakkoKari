...
public class ChannelPlusRecorder : IRecorderProcess
{
    ...
    private readonly bool isPlayOnlyMode = false;

    //RecordingManager rm = null;
    private readonly Record rec = null;
    private readonly string url;
    private readonly Curl videoPageCurl = new();

    //string auth = null;
    private KeyValuePair<string, string>? hlsUrl;
    //RecordInfo ri = null;
    ...
    public int run()
    {
        ...
        {
            rm.form.addLogText(
                "DLLファイルの読み込みに失敗しました。Visual Studio2015のVisual C++再頒布可能パッケージをインストールすると読み込めるようになるかもしれません。");
        ...
        while (rm.rfu == rfu)
        {
            ...
            var isEnded = vpRes.IndexOf("\"live_finished_at\":null") == -1 ||
                          vpRes.IndexOf("\"live_scheduled_start_at\":null") > -1;
            ...
            if (pageType == 0 || pageType == 7) return _rec(vpRes);
    ...
    private bool setParams(string res)
    {
        /*
    ...
    */
    ...
    private KeyValuePair<string, string>? getM3u8Url(string res, string sessionId)
    {
        try
        {
            ...
            if (names.Count != urls.Count)
            {
                ...
                return urls.Count == 0
                    ? null
                    : new KeyValuePair<string, string>("_", urls[0].Value);
            }
    ...
    private void setFormInfo(string res)
    {
        ...
        try
        {
            ...
            Task.Run(() => { rss.set(ri.si.data + "\"watchPageUrl\":\"" + url + "\"", ri.si.recFolderFileInfo, res); });
        }
        catch (Exception e)
        {
            util.debugWriteLine(e.Message + e.Source + e.StackTrace);
    ...
    private void saveComment(string vpRes)
    {
        ...
        try
        {
            ...
            {
                if (tscg == null)
                    tscg = new TimeShiftCommentGetter_chPlus(form, rm, rfu, ri, id, group_id, vpRes, true,
                        ri.timeShiftConfig, this, rl);
                tscg.save();
            }
        }
        catch (Exception e)
    ...
    public override void reConnect(WebSocket ws)
    {
    }
    ...
    public void displaySchedule()
    {
        try
        {
            ...
            while (rm.rfu == rfu && IsRetry)
            {
                ...
                var _keikaTimeStart = tsHlsRequestTime - tsStartTime - jisa;
...
{
    private readonly Curl curl = new();
    ...
    private bool writeM3u8(string[] lines, int i, int port, string lastUrl, Stream sw)
    {
            ...
            if (!l.StartsWith("#EXTINF"))
            {
                ...
                else
                {
                    ...
                    else if (l.StartsWith("#EXT-X-KEY:METHOD="))
                    {
                        ...
                        bodyBuf += "#EXT-X-KEY:METHOD=AES-128,URI=\"http://127.0.0.1:" + port
                            + "/" + HttpUtility.UrlEncode(uri) + "\r\n";
                        util.debugWriteLine("#EXT-X-KEY:METHOD=AES-128,URI=\"http://127.0.0.1:" + port + "/" +
                                            HttpUtility.UrlEncode(uri));
                    }
                    else if (l.StartsWith("http") && isWrite)
                    {
                        //sw.WriteLine("http://127.0.0.1:" + port.ToString() + "/" + HttpUtility.UrlEncode(l));
                        bodyBuf += "http://127.0.0.1:" + port + "/" + HttpUtility.UrlEncode(l) + "\r\n";
                        util.debugWriteLine("http://127.0.0.1:" + port + "/" + HttpUtility.UrlEncode(l));
                        lastWriteUrl = l;
                    }
    ...
    private void setKeikaJikan(double startSeconds)
    {
        ...
        rm.form.setKeikaJikan(keikaJikan, timeLabelKeika + "/" + programTimeStr,
            _keikaJikanDt.ToString("h'時間'mm'分'ss'秒'"), DateTime.Now);
    }

    private string[] getTimeFilterRes(string res, string[] lines, out string[] tsUrlList)
    {
        tsUrlList = null;
        try
        {
    ...
    private void runServer(int port, string[] lines, string[] tsUrlList)
    {
        ...
        try
        {
            ...
            listener.Start();
            rm.form.addLogText("http://127.0.0.1:" + port + "/segment.m3u8");
