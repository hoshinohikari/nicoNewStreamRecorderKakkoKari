...
public class TimeShiftCommentGetter_xml
{
    ...
    private readonly List<string> gotCommentList = new();
    private readonly string[] gotMinXml = new string[2];

    private readonly bool isGetXml;
    private readonly bool isRtmp;

    private readonly bool isTimeShift = true;

//		public TimeShiftConfig tsConfig;
    private readonly bool isVposStartTime;
    private readonly string lvid;
    private readonly long openTime;
    private readonly string programType = null;
    ...
    private int gotMinTime;
    private long gotMinVpos;
    public bool isEnd;

    //bool isSave = true;
    private bool isRetry = true;
    private int lastGotMinTime;
    private long lastGotMinVpos;
...
//						if (isBreak) break;
                    ...
                    for (var j = i + 0; j < b.Length && j < lastI && i > -1; j++) b[j] = 0;
                    ...
                    if (!isKaburiNasi)
                    {
                        util.debugWriteLine("kaburinasi last_res " + lastLastRes + " gotCount " +
                                            gotCommentList.Count);
                        Task.Run(() =>
                        {
                            Thread.Sleep(1000);
                            if (lastLastRes < 900)
...
//				util.debugWriteLine("write comment " + s);
        ...
        {
            util.debugWriteLine("comment write exception " + ee.Message + " " + ee.StackTrace);
        }
...
//				addDisplayComment(chatinfo);
    ...
    private byte[] getRequest()
    {
        ...
        var reqStr = "<thread thread=\"" + thread +
                     "\" version=\"20061206\" res_from=\"-1000\" fork=\"0\" user_id=\"" + userId +
                     "\" with_global=\"1\" scores=\"1\" nicoru=\"0\" waybackkey=\"" + waybackKey + "\" when=\"" +
                     when + "\" /> \0";
...
//				util.debugWriteLine(util.getRegGroup(c, "date.+?(\\d+)"));
        ...
        for (var j = 1; j < fileNum + 1; j++)
        {
            ...
            using (var w = new StreamWriter(fileName + "_", false, Encoding.UTF8))
            {
                    ...
                    if (quePosTimeList != null)
                    {
                        ...
                        if (j == fileNum - 0 || vpos < quePosTimeList[j + 0])
                        {
                            ...
                            var newVposPart = oriVposPart.Replace(vpos.ToString(),
                                (vpos - quePosTimeList[j - 1]).ToString());
                ...
                if (isGetXml) w.WriteLine("</packet>");
                //w.Close();
            }
    ...
    private void heartBeater()
    {
        while (rm.rfu == rfu)
        {
            try
            {
                ...
                var contentStr =
                    "{\"lang\":\"ja-jp\",\"locale\":\"JP\",\"seat_locale\":\"JP\",\"screen\":\"ScreenNormal\",\"v\":\"" +
                    lvid + "\",\"datarate\":0}";
