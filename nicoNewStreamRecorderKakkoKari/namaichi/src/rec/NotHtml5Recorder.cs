...
public class NotHtml5Recorder
{
    ...
    {
        var getPlayerStatusInfo = getGetPlayerStatusInfo();
                   ...
                   "(<archive>0</archive>)") != null && rm.rfu == rfu)
        {
            ...
            var recFolderFile = getNotHtmlRecFolderFile(res, getPlayerStatusInfo);
            if (recFolderFile == null) return;
            util.debugWriteLine("recff " + string.Join("  ", recFolderFile));
            var command = getGetPlayerStatusCommand(getPlayerStatusInfo, recFolderFile);
            if (command == null) return;
            util.debugWriteLine("command " + string.Join("  ", command));
            var messageInfo = getMessageInfo(getPlayerStatusInfo);
            var isFFmpeg = Array.IndexOf(command, "-i") != -1;
...
//			util.debugWriteLine(res);
    ...
    private string[] getNotHtmlRecFolderFile(string res, string getPlayerStatusInfo)
    {
        ...
        var isChannel = getPlayerStatusInfo.IndexOf("<provider_type>channel</provider_type>") < 0 ? false : true;
        ...
        if (!isOfficial && !isChannel)
        {
            ...
            group = HttpUtility.UrlDecode(group);
        ...
        else
        {
            ...
            communityNum = isOfficial ? "official" : util.getRegGroup(res, ",\"group_id\":\"(.*?)\"");
            host = util.getRegGroup(res, "class=\"company\" title=\"(.*?)\"");

            if (isOfficial)
            {
                var cn = util.getRegGroup(res, "<a href=\".*?(ch[0-9]*)\".*class=\"ch_name\"");
...
//			string title = util.getRegGroup(res, "Nicolive_JS_Conf.Watch = \\{.*?\"videoTitle\":\"(.*?)\"");
        var title = util.getRegGroup(getPlayerStatusInfo, "<title>(.+)</title>");
        var lvid = util.getRegGroup(url, "(lv[0-9]+)");

        util.debugWriteLine(
            "host group title " + host + " " + group + " " + title + " " + lvid + " " + communityNum);
    ...
    private string[] getGetPlayerStatusCommand(string getPlayerStatusInfo, string[] recFolderFile)
    {
        ...
        var isOfficial = getPlayerStatusInfo.IndexOf("<provider_type>official</provider_type>") < 0 ? false : true;
        var getPlayerStatusCommandVal = getPlayersStatusCommandVal(isOfficial, getPlayerStatusInfo);
    ...
    private string[] getPlayersStatusCommandVal(bool isOfficial, string getPlayerStatusInfo)
    {
        if (!isOfficial)
        {
            ...
            var ticket = util.getRegGroup(getPlayerStatusInfo, "<ticket>(.+?)</ticket>");
            var contentsUrl = util.getRegGroup(getPlayerStatusInfo, "<contents_list>.+?(rtmp://.+?)<");
            var rtmpUrl = util.getRegGroup(getPlayerStatusInfo, "<url>(.+?)</url>");
            var lvid = util.getRegGroup(getPlayerStatusInfo, "<id>(.+?)</id>");
        ...
        var contentsCases = util.getRegGroup(getPlayerStatusInfo, "<contents .+?>case:(.+?)</contents>");
        ...
        try
        {
            decodedContentsCases = HttpUtility.HtmlDecode(contentsCases);
            ...
            var tickets = util.getRegGroup(getPlayerStatusInfo, "<tickets>(.+?)</tickets>");
            var ticketUid = util.getRegGroup(tickets, premiumContentsUrl1 + "\">(uid=.+?)</stream>");
            ...
            if (ticketUid != null)
            {
                ...
                var url = "\"" + premiumContentsUrl0 + "/" + premiumContentsUrl1 + "?" + decodedTickets + "\"";
            ...
            else
            {
                //					String decodedContentsCases = null, decodedTickets = null;
                var ticket = util.getRegGroup(getPlayerStatusInfo, "<ticket>(.+?)</ticket>");
                var stream = util.getRegGroup(getPlayerStatusInfo,
                    "<stream name=\"" + premiumContentsUrl1 + "\">(.*?)</stream>");
                ...
                var url = "\"" + premiumContentsUrl0 + "/" + premiumContentsUrl1 + "?" + stream + " live=1\"";
    ...
    {
        var val = getPlayerStatusCommandVal;
        if (!isOfficial)
        {
            //0-ticket 1-contentsUrl 2-rtmpUrl 3-lvid
            string[] command =
    ...
    {
        var url = util.getRegGroup(getPlayerStatusInfo, "<addr>(.+?)</addr>");
        var port = util.getRegGroup(getPlayerStatusInfo, "<port>(.+?)</port>");
        var thread = util.getRegGroup(getPlayerStatusInfo, "<thread>(.+?)</thread>");
        return new[] { url, port, thread };
    }
}

internal class NotHtmlCommentGetter
{
    ...
    public NotHtmlCommentGetter(string[] messageInfo, RecordingManager rm, RecordFromUrl rfu,
