...
public class FollowCommunity
{
        ...
        bool isPlayOnlyMode)
    {
        ...
        var comId = isJikken
            ? util.getRegGroup(res, "&quot;followPageUrl&quot;\\:&quot;.+?motion/(.+?)&quot;")
            :
    ...
    private bool join2(string comId, CookieContainer cc, MainForm form, config.Config cfg, bool isPlayOnlyMode)
    {
        ...
        try
        {
            ...
            using (var sr = new StreamReader(rs))
            {
                ...
                form.addLogText(isSuccess ? "フォローしました。" + _m + "開始までしばらくお待ちください。" : "フォローに失敗しました。");
                return isSuccess;
            }
        }
        catch (Exception e)
...
//					util.debugWriteLine(cc.GetCookieHeader(new Uri(url)));
            ...
            try
            {
                ...
                var data =
                    "mode=commit&title=" + HttpUtility.UrlEncode("フォローリクエスト", enc);
                var postDataBytes = Encoding.ASCII.GetBytes(data);
