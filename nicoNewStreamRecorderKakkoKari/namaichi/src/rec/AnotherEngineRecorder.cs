...
//			_command = _command.Replace("{o}", o);
    ...
    private void renameStatistics(string recFolderFile)
    {
        try
        {
            Task.Run(() =>
            {
                ...
                var newName = recFolderFile.Replace("{w}", wsr.visitCount.Replace("-", ""))
                    .Replace("{c}", wsr.commentCount.Replace("-", "")) + ext;
    ...
    {
        var res = util.getPageSource(hlsSegM3uUrl);
        ...
        double sum = 0;
        foreach (Match _m in m) sum += double.Parse(_m.Groups[1].Value);
