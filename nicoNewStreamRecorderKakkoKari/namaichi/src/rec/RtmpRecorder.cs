...
{
    private readonly int afterConvertMode;
...
//		/private config.config cfg;
    ...
    public int retryMode; //0-retry 1-stop 2-endProgram

    private Process rtmpdumpP;

    private string rtmpUrl;

    public int subNtiGroupNum;

    //private string que;
    private string ticket;
    private int tsRecordIndex;
    private int tsRecordNum;
...
//			this.cfg = cfg;
    ...
    public void record(string rtmp2Url, string quality)
    {
        ...
        try
        {
            ...
            while (rm.rfu == rfu && retryMode == 0)
            {
                ...
                {
                    while (rm.rfu == rfu && retryMode == 0) Thread.Sleep(1000);
                    return;
                }
                ...
                if (!isSub)
                {
                    ...
                    {
                        //end program
                        while (rm.rfu == rfu && !rtmpdumpP.HasExited) Thread.Sleep(1000);
                    }
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (afterConvertMode > 0) convertList.Add(recFolderFile + ".flv");
                ...
                else
                {
                    ...
                    if (rm.rfu != rfu || retryMode == 1) stopRecording();
                    if (retryMode == 2)
                    {
                        //end program
                        ...
                        while (rm.rfu == rfu)
                        {
                            if ((rtmpdumpP == null || rtmpdumpP.HasExited) &&
                                (ffmpegP == null || ffmpegP.HasExited)) break;
                            Thread.Sleep(1000);
                        }
                    }
    ...
    private string getProcessArgs(bool isRtmp2, bool isFirst)
    {
              ...
              )
        {
            ...
            if (pageType == 2 || pageType == 3) return "no";
...
//					string ticket = null;
                if (_ticket != null)
                {
                    ...
                    if (name != null)
                    {
                        ...
                        if (!isSub && !rfu.isPlayOnlyMode)
                            arg += " -o \"" + util.getOkSJisOut(recFolderFile) + ".flv\"";
...

            {
                ...
                */
                util.debugWriteLine(type + " contentsUrl " + contentsUrl + " rtmpUrl " + rtmpUrl + " ticket " +
                                    ticket + util.getMainSubStr(isSub, true));
...
//						string _out = (arg == "") ? util.getOkSJisOut(recFolderFile[1]) : wr.getRecFilePath(0)[1];
                ...
                arg += "-r " + rtmpUrl + _y + a + " -a " + app + " -p http://live.nicovideo.jp/watch/" + lvid +
                       " -s \"http://live.nicovideo.jp/nicoliveplayer.swf\" -f \"WIN 29,0,0,113\" -t " + rtmpUrl +
                       " -C S:" + ticket + _a + " -m 300 -o ";
        ...
        else
        {
            ...
            {
                if (arg != "") arg += "$";
                arg += "-vr " + rtmpUrl + " -N " + a + " -C S:" + ticket + " -p http://live.nicovideo.jp/watch/" +
                       lvid +
                       " -s http://live.nicovideo.jp/nicoliveplayer.swf?180116154229 -f \"WIN 29,0,0,113\" " +
                       " -o ";
            }
...
//					var apiRes = util.getPageSource(url + "/programinfo", ref a, container);
    ...
    private void getProcess(out Process rtmpdumpP, out Process ffmpegP, string rtmpdumpArg)
    {
        ...
        Directory.SetCurrentDirectory(util.getJarPath()[0]);
        rtmpdumpP = ffmpegP = null;
        rtmpdumpP = new Process();
        var si = new ProcessStartInfo();
        si.FileName = bool.Parse(rm.cfg.get("IsDefaultRtmpPath")) ? "rtmpdump.exe" : rm.cfg.get("rtmpPath");
...
//			var _is = ffmpegP.StandardInput.BaseStream;
        ...
        while (!rtmpdumpP.HasExited && rfu == rm.rfu)
            try
            {
                ...
                if (i == 0) util.debugWriteLine("rtmpdump read i " + " get 0");
            ...
            {
                Debug.WriteLine("rtmpdump read exception " + ee.Message + ee.Source + ee.StackTrace +
                                ee.TargetSite);
            }
...
//			Process rtmpdumpP, ffmpegP;
        ...
        try
        {
            for (tsRecordIndex = 0; tsRecordIndex < argList.Length; tsRecordIndex++)
            {
                ...
                if (_arg.StartsWith("-r")) makeTs(_arg);
...
//						   		(afterConvertMode != "0" && afterConvertMode != "4")) {
    ...
    private void _stopRecording(Process p)
    {
        try
        {
            util.debugWriteLine("stop recording rtmp p " + p + " p.hasexited= " +
                                (p == null ? "" : p.HasExited.ToString()));
            ...
            catch (Exception eee)
            {
                util.debugWriteLine(eee.Message + eee.StackTrace + eee.Source + eee.TargetSite +
                                    util.getMainSubStr(isSub, true));
            }

            while (!p.HasExited) Thread.Sleep(200);
