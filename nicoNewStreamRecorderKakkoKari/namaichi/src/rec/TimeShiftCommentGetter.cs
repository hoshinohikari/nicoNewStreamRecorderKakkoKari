...
{
    private readonly string commentConvertStr;
    ...
    private readonly bool isGetXml = true;

//		public TimeShiftConfig tsConfig;
    //private bool isVposStartTime;
    private readonly bool isRtmp;

    //long openTime = 0;
    //long _openTime = 0;
    //string recFolderFile;
    private readonly string lvid;
    ...
    private readonly RtmpRecorder rr;
    private readonly string thread;
    private readonly string threadStore;
    private readonly string uri;
    private readonly string uriStore;
    ...
    private int lastLastRes = int.MaxValue;
    ...
    )
    {
        ...
        isGetXml = bool.Parse(rm.cfg.get("IsgetcommentXml"));
        ...
        isVposStartTime = ri.isRtmp ? false : tsConfig.isVposStartTime;
        this.isRtmp = isRtmp;
        this.rr = rr;
    ...
    private bool connect()
    {
        ...
        try
        {
            ...
            header.Add(new KeyValuePair<string, string>("Sec-WebSocket-Extensions",
            ...
            wsc = new WebSocket(uri, "", null, header, util.userAgent, "", WebSocketVersion.Rfc6455, null,
            ...
            try
            {
                ...
                if (wsc != null && wsc.State == WebSocketState.Connecting)
                {
                    ...
                    {
                        util.debugWriteLine("tscg connect timeout ws exception " + e.Message + e.Source +
            ...
            {
                util.debugWriteLine("tscg ws connect exception " + ee.Message + ee.Source + ee.StackTrace +
                                    ee.TargetSite);
                return false;
            }
    ...
    private void onWscOpen(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            _gotMinXml = new[] { gotMinXml[0], gotMinXml[1] };
...
//					rm.form.addLogText("コメントの保存を完了しました");
    ...
    {
        util.debugWriteLine("ms tscg onerror " + e.Exception.Message + e.Exception.Source + e.Exception.StackTrace +
    ...
    private void onWscMessageReceive(object sender, MessageReceivedEventArgs e)
    {
        ...
        try
        {
            if ((rm.rfu != rfu && lastRealTimeComment == null) || !isRetry)
            {
                ...
                {
                    util.debugWriteLine("wsc message receive exception " + ee.Source + " " + ee.StackTrace + " " +
                ...
                if (chatinfo.root == "thread")
                {
                    if (threadLine == null)
                    {
                        ...
                        if (!rfu.isPlayOnlyMode && isLog)
                            form.addLogText("アリーナ席に" + chatinfo.lastRes + "件ぐらいの" + (isStore ? "ストア" : "") +
                                            "コメントが見つかりました(追い出しコメント含む)");
                    }
                }
                else
                {
            ...
            if (eMessage.IndexOf("rf:") > -1) ((WebSocket)sender).Close();
            //if (!isTimeShift)
...
//			var when = (isSave) ? lastGetChatTime.ToString() : openTime.ToString();
        ...
        var ret =
            ...
            thread + "\",\"version\":\"20061206\",\"fork\":0,\"user_id\":\"" + userId + "\",\"res_from\":" +
            resfrom + ",\"with_global\":1,\"scores\":1,\"nicoru\":0,\"waybackkey\":\"" + waybackKey +
            "\",\"when\":" + when + "}},{\"ping\":{\"content\":\"pf:0\"}},{\"ping\":{\"content\":\"rf:0\"}}]";
    ...
    */
    private void endProcess()
    {
        ...
        try
        {
            ...
            while (isRtmp && (rr == null || rr.fileNameList == null)) Thread.Sleep(1000);
            ...
            gotCommentList.Sort(commentListCompare);
            ...
            for (var j = 0; j < fileNum; j++)
            {
                ...
                if (rm.cfg.get("fileNameType") == "10")
                    fileName = fileName.Replace("{w}", rp.visitCount).Replace("{c}", rp.commentCount);
                using (var w = new StreamWriter(fileName + "_", false, Encoding.UTF8))
                {
                    if (isGetXml)
                    {
                    ...
                    for (var i = 0; i < chats.Length; i++)
                        if (quePosTimeList != null)
                        {
                            ...
                            if (j == fileNum - 1 || vpos < quePosTimeList[j + 1])
                            {
                                ...
                                var newVposPart = oriVposPart.Replace(vpos.ToString(),
                                    (vpos - quePosTimeList[j]).ToString());
...
//			if (publishList.Count == 0) quePosTimeList = new int[]{0};
    ...
    private string getPlay(XmlNode ques)
    {
        ...
        foreach (XmlNode q in ques.ChildNodes)
        {
            ...
            else if (qi.StartsWith("/play"))
            {
                return util.getRegGroup(qi, "rtmp:(.+?)[, $]");
            }
        }

        if (premiumP != null) return premiumP;
        if (defaultP != null) return defaultP;
        return null;
    }
    ...
    */
    private void saveStore()
    {
        ...
        var tscg = new TimeShiftCommentGetter(uri, thread,
            ...
            //startSecond, 
            //isVposStartTime, 
            isRtmp, rr, roomName, tsConfig, null, true, true);
        tscg.save();

        while (!tscg.isEnd && rm.rfu == rfu && isRetry && rp.IsRetry) Thread.Sleep(500);
