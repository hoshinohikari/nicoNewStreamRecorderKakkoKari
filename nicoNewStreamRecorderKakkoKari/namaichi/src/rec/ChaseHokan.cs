...
public class ChaseHokan
{
    ...
    public void start()
    {
        ...
        if (wr == null) rm.form.addLogText("補完の設定に失敗しました");
        try
        {
            wr.start();
            if (wr.isEndProgram)
    ...
    private WebSocketRecorder getWebsocketRecorder(string res)
    {
        try
        {
            ...
            //var n = nti;
            var lastWroteSecondsAgo = (int)((DateTime.Now - nti.dt).TotalSeconds +
                                            (int)((nti.no - lastSegmentNo) * nti.second) + 25) * -1;
            var endSecondsAgo = (int)((DateTime.Now - nti.dt).TotalSeconds - 15) * -1;
            var tsConfig = new TimeShiftConfig(0, 0, 0, lastWroteSecondsAgo, 0, 0, endSecondsAgo, false, false, "",
                false, 0, false, false, 1, 1, false, false, false, qualityRank);
            var recFolderFile = new[] { this.ri.recFolderFile[0], name[1], null };
