...
public class ChaseLastRecord
{
    ...
    private readonly string[] recFolderFileInfo;
    ...
    private readonly TimeShiftConfig tsConfig;
    ...
    public void rec(string recFolderFile)
    {
        ...
        if (res == null)
        {
            if (int.Parse(rm.cfg.get("afterConvertMode")) > 0) ffmpegThrough(recFolderFile);
            return;
        }
    ...
    private string getRes()
    {
        for (var i = 0; i < 12 && rm.rfu == rfu; i++)
        {
            ...
            var timeout = 3000;
            var _res = util.getPageSource("https://live.nicovideo.jp/watch/" + lvid, container, null, false,
                timeout);
            if (_res == null)
            {
    ...
    private WebSocketRecorder getWebsocketRecorder(string res)
    {
        try
        {
            ...
            var lastFile = util.getLastTimeshiftFileName(
                recFolderFileInfo[0], recFolderFileInfo[1], recFolderFileInfo[2], recFolderFileInfo[3],
                recFolderFileInfo[4], recFolderFileInfo[5], rm.cfg, openTime, this.ri.isFmp4);
            util.debugWriteLine("timeshift lastfile " + lastFile);
            var lastFileTime = util.getLastTimeShiftFileTime(lastFile, segmentSaveType, this.ri.isFmp4);
            if (lastFileTime == null)
                util.debugWriteLine("timeshift lastfiletime " +
                                    (lastFileTime == null ? "null" : string.Join(" ", lastFileTime)));
            var tsConfig = new TimeShiftConfig(1, int.Parse(lastFileTime[0]), int.Parse(lastFileTime[1]),
                int.Parse(lastFileTime[2]), 0, 0, 0, true, false, "", false, 0, false, false, 2, 0, false, false,
                this.tsConfig.isDeletePosTime, this.tsConfig.qualityRank);
            ...
            var recFolderFile = util.getRecFolderFilePath(recFolderFileInfo[0], recFolderFileInfo[1],
                recFolderFileInfo[2], recFolderFileInfo[3], recFolderFileInfo[4], recFolderFileInfo[5], rm.cfg,
                true, tsConfig, openTime, false, this.ri.isFmp4);
    ...
    private void ffmpegThrough(string recFolderFile)
    {
