...
{
    private readonly object commentLock = new();

    private readonly List<string> debugWriteBuf = new();
    private readonly bool isConvertSpace;
    private readonly string isGetComment;
    private readonly bool isGetCommentXml;
    private readonly bool isGetCommentXmlInfo;

    //public List<string> chaseCommentBuf = new List<string>();

    private readonly bool isNotSleep;

    //public List<string[]> commentReplaceList = null;
    private readonly bool isSaveCommentOnlyRetryingRec;
    private readonly List<string> lastSaveComments = new();

    private readonly string[] qualityRank;
    private readonly WebSocket[] wsc = new WebSocket[2];
    private readonly List<WebSocket> wsList = new();

    //private bool isXmlComment = true;
    private readonly XmlCommentGetter_ontime xcg = null;
    ...
    private bool isFirstCommentAfterOpenWsc;
...
//		private System.Threading.Thread mainThread;
    ...
    public RecordStateSetter rss;
    private string sendCommentBuf;
    private string threadkey;

    //public long serverTime;
    private string ticket;
    private TimeShiftCommentGetter_xml tscgx;

    private Task tsWriterTask;
    ...
    */
        ...
        RecordStateSetter rss, RecordInfo ri)
    {
        ...
        qualityRank = ri.timeShiftConfig == null || ri.timeShiftConfig.qualityRank == null
            ? rm.cfg.get("qualityRank").Split(',')
            : ri.timeShiftConfig.qualityRank;
        isGetComment = rm.cfg.get("IsgetComment");
        isGetCommentXml = bool.Parse(rm.cfg.get("IsgetcommentXml"));
        isGetCommentXmlInfo = bool.Parse(rm.cfg.get("IsgetcommentXmlInfo"));
        engineMode = rm.cfg.get("EngineMode");
        isNotSleep = bool.Parse(rm.cfg.get("IsNotSleep"));
    ...
    public bool start()
    {
        ...
        {
            if (ri.isRtmp && ri.si.isTimeShift) Task.Run(() => rtmpRecord(null, null));
            connect();
        ...
        {
            if (!ri.isRtmp && ws.State == WebSocketState.Closed) addDebugBuf("no connect loop ws close");
            //					connect();
            if (ws != null)
            {
                    ...
                    && ws == lastWebSocket)
                {
                    ...
                    if (stopWsCount > 10)
                    {
                        addDebugBuf("stop ws count " + stopWsCount + " close ws / rec.isreconnecting " +
                                    (rec != null ? rec.isReConnecting.ToString() : ""));
...
//				wsc.Close();
    ...
    private bool connect()
    {
        ...
        try
        {
            ...
            ws = new WebSocket(ri.webSocketRecInfo[0], "", null, null, util.userAgent, "", WebSocketVersion.Rfc6455,
                null, SslProtocols.Tls12 | SslProtocols.Tls11 | SslProtocols.Tls);
...
//				var _ws = ws; 
            ...
            if (_ws != null && _ws.State == WebSocketState.Connecting)
            {
                ...
                {
                    addDebugBuf(
                        "connect timeout ws exception " + e.Message + e.Source + e.StackTrace + e.TargetSite);
                }
            }
        }
        catch (Exception ee)
...
//			if (rm.rfu != rfu) stopRecording();
    ...
    private void onClose(object sender, EventArgs e)
    {
        ...
        addDebugBuf("on close " + e + " ws hash " + sender.GetHashCode() + " istimeshift " + ri.si.isTimeShift +
                    " wsList " + wsList.Count);
        try
        {
...
//				wsList.Remove((WebSocket)sender);
        ...
        if (rm.rfu == rfu && !isEndProgram && (WebSocket)sender == ws) connectUntilOk();
        //				ws.Open();
    }
...
//			endStreamProcess();
    ...
    private void onMessageReceive(object sender, MessageReceivedEventArgs e)
    {
        ...
        var type = util.getRegGroup(e.Message,
            "\"" + (ri.webSocketRecInfo[2] == "1" && e.Message.IndexOf("\"command\"") > -1 ? "command" : "type") +
            "\":\"(.+?)\"");
...
//			addDebugBuf("ws " + ws);
        ...
        if (type == "stream" || type == "currentstream")
        {
            ...
            if (ri.si.isRtmpOnlyPage) return;
            //isRtmp || 
            ...
            if (ri.isChase && !isChaseStream(e.Message))
            {
                var chaseReq = ri.webSocketRecInfo[2] == "1"
                    ...
                    : "{\"type\":\"changeStream\",\"data\":{\"quality\":\"" + bestGettableQuolity +
                      "\",\"protocol\":\"hls\",\"latency\":\"low\",\"chasePlay\":true}}";
                sendMessage(ws, chaseReq);
                util.debugWriteLine("chase sent " + chaseReq);
                return;
            }

            if (ri.isChase) addDebugBuf("got chaseStream");
...
//					displaySchedule();
        ...
        if (e.Message.IndexOf("\"NO_STREAM_AVAILABLE\"") >= 0) rm.form.addLogText("配信データが取得できませんでした");
        if (type == "postkey")
    ...
    {
        //Task.Run(() => {
        var keikaTimeStart = DateTime.MinValue;
        while (rm.rfu == rfu && IsRetry)
        {
            ...
            var _keikaTimeStart = !ri.si.isTimeShift || ri.isChase
                ? util.getUnixToDatetime(ri.si.openTime) - jisa
                : tsHlsRequestTime - tsStartTime - jisa;
            ...
            rm.form.setKeikaJikan(keikaJikan, timeLabelKeika + "/" + programTimeStr,
                _keikaJikanDt.ToString("h'時間'mm'分'ss'秒'"), _keikaTimeStart);
            Thread.Sleep(1000);
        }
        //});
    }
    ...
    */
    private void sendPong()
    {
        try
        {
            if (ri.webSocketRecInfo[2] == "1")
            {
                ...
                sendMessage(ws,
                    "{\"type\":\"watch\",\"body\":{\"command\":\"watching\",\"params\":[\"" +
                    ri.webSocketRecInfo[3] + "\",\"-1\",\"0\"]}}");
            }
            else
            {
    ...
    {
        var hlsUrl = util.getRegGroup(message, "\"uri\":\"(.+?)\"");
        ;
    ...
    private void connectMessageServer(WebSocket _ws)
    {
        ...
        wsc[0] = new WebSocket(msUri, "", null, header, util.userAgent, "", WebSocketVersion.Rfc6455, null,
...
//			himodukeWS[1] = wsc;
        ...
        if (msStoreUri != null)
        {
            wsc[1] = new WebSocket(msStoreUri, "", null, header, util.userAgent, "", WebSocketVersion.Rfc6455, null,
                SslProtocols.Tls12 | SslProtocols.Tls11 | SslProtocols.Tls);
    ...
    {
        msUri = util.getRegGroup(msg,
            ri.webSocketRecInfo[2] == "1" ? "messageServerUri\"\\:\"(ws.+?)\"" : "\"uri\":\"(ws.+?)\"");
        ...
        msReq = new[]
        {
            ...
            msThread + "\",\"version\":\"20061206\",\"fork\":0,\"user_id\":\"" + ri.userId + "\",\"res_from\":" +
            res_from + ",\"with_global\":1,\"scores\":1,\"nicoru\":0" +
            (threadkey == null ? "" : ",\"threadkey\":\"" + threadkey + "\"") + "}" +
            "},{\"ping\":{\"content\":\"pf:0\"}},{\"ping\":{\"content\":\"rf:0\"}}]"
        };

        if (bool.Parse(rm.cfg.get("IsgetcommentStore")))
        {
            msStoreUri = util.getRegGroup(msg,
                ri.webSocketRecInfo[2] == "1" ? "messageServerUri\"\\:\"(ws.+?)\"" : "\"uri\":\"(ws.+?)\"");
            msStoreThread = util.getRegGroup(msg, "threadId\":\"(.+?)\"");
            msStoreReq = new[]
            {
                "[{\"ping\":{\"content\":\"rs:0\"}},{\"ping\":{\"content\":\"ps:0\"}},{\"thread\":{\"thread\":\"" +
                msThread + "\",\"version\":\"20061206\",\"fork\":0,\"user_id\":\"" + ri.userId +
                "\",\"res_from\":" + res_from +
                ",\"with_global\":1,\"scores\":1,\"nicoru\":0}},{\"ping\":{\"content\":\"pf:0\"}},{\"ping\":{\"content\":\"rf:0\"}}]"
            };
        }

        var _roomName = util.getRegGroup(msg,
            ri.webSocketRecInfo[2] == "1" ? "\"roomName\":\"(.+?)\"" : "\"name\":\"(.+?)\"");
    ...
    public void stopRecording(WebSocket _ws, WebSocket[] _wsc)
    {
        ...
        try
        {
            if (_ws != null && _ws.State != WebSocketState.Closed)
                ...
                {
                    addDebugBuf("stoprecording ws close exception " + e.Message + e.Source + e.StackTrace +
                                e.TargetSite);
                }
            //				_ws = null;
        }
        ...
        try
        {
            if (_wsc[0] != null && _wsc[0].State != WebSocketState.Closed &&
                _wsc[0].State != WebSocketState.Closing)
            {
        ...
        try
        {
            if (_wsc[1] != null && _wsc[1].State != WebSocketState.Closed &&
                _wsc[1].State != WebSocketState.Closing)
            {
...
//				if (commentSW != null) commentSW.Close();
    ...
    */
    private void displayStatistics(string e)
    {
        ...
        if (ri.webSocketRecInfo[2] == "1")
        {
            _visit = util.getRegGroup(e,
                "{\"type\":\"watch\",\"body\":{\"command\":\"statistics\",\"params\":\\[\"(\\d+?)\",\"\\d+?\"");
            _comment = util.getRegGroup(e,
                "{\"type\":\"watch\",\"body\":{\"command\":\"statistics\",\"params\":\\[\"\\d+?\",\"(\\d+?)\"");
        }
        else
        {
    ...
    private void onWscOpen(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (bool.Parse(isGetComment) && commentSW == null && !rfu.isPlayOnlyMode && sender == wsc[0])
            {
                ...
                {
                    var __commentFileName = util.getOkCommentFileName(rm.cfg, commentFileName, ri.si.lvid,
                        ri.si.isTimeShift, ri.isRtmp);
                    try
                    {
                ...
                var _commentFileName =
                    util.getOkCommentFileName(rm.cfg, fName, ri.si.lvid, ri.si.isTimeShift, ri.isRtmp);
                var isExists = File.Exists(_commentFileName);
                commentSW = new StreamWriter(_commentFileName, false, Encoding.UTF8);
    ...
    {
        while (_wsc.State == WebSocketState.Open && !isEndProgram && IsRetry)
            try
            {
                //_wsc.Send("");
                sendMessage(_wsc, "");
    ...
    private void onWscClose(object sender, EventArgs e)
    {
        ...
        try
        {
            if (rm.rfu == rfu && IsRetry && !isTimeShiftCommentGetEnd)
                ...
                {
                    addDebugBuf("on wsc close ws close exception " + ee.Message + ee.Source + ee.StackTrace +
                                ee.TargetSite);
                }
        }
        catch (Exception ee)
            ...
            try
            {
                if (rm.cfg.get("fileNameType") == "10" && (name.IndexOf("{w}") > -1 || name.IndexOf("{c}") > -1))
                    Task.Run(() =>
                    {
                        ...
                        var _name = name.Replace("{w}", visitCount.Replace("-", ""))
                            .Replace("{c}", commentCount.Replace("-", ""));
...
//			endStreamProcess();
    ...
    private void onWscError(object sender, ErrorEventArgs e)
    {
...
//			endStreamProcess();
    ...
    private void onWscMessageReceive(object sender, MessageReceivedEventArgs e)
    {
        ...
        if (lastSaveComments.Count > 0 && isFirstCommentAfterOpenWsc) checkMissingComment();
        ...
        if (ri.si.isTimeShift && eMessage.StartsWith("{\"ping\":{\"content\":\"rf:") && !ri.isChase &&
            sender == wsc[0])
        {
            closeWscProcess();
            try
            ...
            {
                addDebugBuf(eee.Message + eee.Source + eee.StackTrace + eee.TargetSite);
            }
        ...
        if (rm.rfu != rfu || ((!ri.si.isTimeShift || ri.isChase) && !IsRetry))
        {
            ...
            {
                addDebugBuf("wsc0 message receive exception " + ee.Source + " " + ee.StackTrace + " " +
                            ee.TargetSite + " " + ee.Message);
            }
            ...
            {
                addDebugBuf("wsc1 message receive exception " + ee.Source + " " + ee.StackTrace + " " +
                            ee.TargetSite + " " + ee.Message);
            }
        ...
        else
        {
            ...
            {
                while (firstSegmentSecond == -1 && rm.rfu == rfu) Thread.Sleep(1000);
        ...
        try
        {
            var isComSave = chatinfo.root == "thread" || ri.timeShiftConfig == null ||
                            ((!ri.timeShiftConfig.isAfterStartTimeComment ||
                ...
                if (commentSW != null)
                {
                    ...
                    else
                    {
                        if (lastSaveComments.IndexOf(writeStr) == -1 &&
                            ...
                            if (!isSaveCommentOnlyRetryingRec || rec == null ||
                                DateTime.Now - rec.lastWroteSegmentDt < TimeSpan.FromSeconds(10))
                            {
        ...
        {
            addDebugBuf("comment write exception " + ee.Message + " " + ee.StackTrace);
        }
    ...
    private void addDisplayComment(ChatInfo chat)
    {
...
//			var bestGettableQuality = getBestGettableQuolity(message);
    ...
    private string getBestGettableQuolity(string msg)
    {
        ...
        var qualityList = config.Config.qualityList.Values
            .Select(x => x.IndexOf("(") > -1 ? util.getRegGroup(x, "\\((.+?)\\)") : x).ToList();
        ...
        var ranks = rm.ri == null ? qualityRank : rm.ri.qualityRank;
        ...
        foreach (var q in newQualities)
        {
            ...
            rm.cfg.set("qualityList", JsonConvert.SerializeObject(config.Config.qualityList));
    ...
    private void sendUseableStreamGetCommand(string bestGettableQuolity)
    {
        ...
        {
            req = ri.isRtmp
                ? "{\"type\":\"watch\",\"body\":{\"command\":\"getstream\",\"requirement\":{\"protocol\":\"rtmp\",\"quality\":\"" +
                  bestGettableQuolity + "\"}}}"
                : "{\"type\":\"watch\",\"body\":{\"command\":\"getstream\",\"requirement\":{\"protocol\":\"hls\",\"quality\":\"" +
                  bestGettableQuolity + "\",\"isLowLatency\":false}}}";
        }
        else
        {
            var _latency = rm.cfg.get("latency");
            var latency = _latency == "新方式の低遅延" || _latency == "1" ? "low" : "high";
            req = "{\"type\":\"changeStream\",\"data\":{\"quality\":\"" + bestGettableQuolity +
                  "\",\"protocol\":\"hls" + (ri.isFmp4 ? "+fmp4" : "") + "\",\"latency\":\"" + latency +
                  "\",\"chasePlay\":" + ri.isChase.ToString().ToLower() + "}}";
        }
...
//			ws.Open();
    ...
    public override void reConnect(WebSocket _ws)
    {
    ...
    public bool isEndedProgram()
    {
        ...
        try
        {
            ...
            if (res.IndexOf("user.login_status = 'not_login'") > -1)
            {
                ...
                var _webSocketInfo = RecordInfo.getWebSocketInfo(res, ri.isRtmp, ri.isChase, ri.si.isTimeShift,
                    rm.form, ri.isFmp4);
                isNoPermission = true;
                addDebugBuf("isendprogram login websocketInfo " + ri.webSocketRecInfo[0] + " " +
                            ri.webSocketRecInfo[1]);
    ...
    public override void sendComment(string s, bool is184)
    {
        if (msThread == null) return;
        sendCommentBuf = s;
        isSend184 = is184;
        var vpos = (int)((DateTime.Now - util.getUnixToDatetime(ri.si.openTime)).TotalMilliseconds / 10) + 60000 +
                   780;
        //ws.Send("{\"type\":\"watch\",\"body\":{\"command\":\"getpostkey\",\"params\":[\"" + msThread + "\"]}}");
        if (ri.webSocketRecInfo[2] == "1")
            sendMessage(ws,
                "{\"type\":\"watch\",\"body\":{\"command\":\"getpostkey\",\"params\":[\"" + msThread + "\"]}}");
        //else sendMessage(ws, "{\"type\":\"getPostkey\"}");
        else
            sendMessage(ws,
                "{\"type\":\"postComment\",\"data\":{\"text\":\"" + s + "\",\"vpos\":" + vpos +
                ",\"isAnonymous\":" + is184.ToString().ToLower() +
                ",\"color\":\"white\",\"size\":\"medium\",\"position\":\"naka\"}}");
    }
    ...
    public void sendCommentWsc(string s)
    {
        ...
        var command =
            "[{\"ping\":{\"content\":\"rs:1\"}},{\"ping\":{\"content\":\"ps:5\"}},{\"chat\":{\"thread\":\"" +
            msThread + "\",\"vpos\":" + vpos + mail + ",\"ticket\":\"" + ticket + "\",\"user_id\":\"" + ri.userId +
            "\",\"content\":\"" + sendCommentBuf + "\",\"postkey\":\"" + postKey + "\", \"premium\":" + premium +
     ...
     */
    public override string[] getRecFilePath()
    {
...
//				return;
        ...
        while ((rm.rfu == rfu && !isLogEnd) || debugWriteBuf.Count > 0)
            try
            {
                lock (debugWriteBuf)
                {
                    var l = new string[debugWriteBuf.Count + 10];
...
//				return;
    ...
    private void resetWebsocketInfo()
    {
        try
        {
            ...
            var _webSocketInfo =
                RecordInfo.getWebSocketInfo(res, ri.isRtmp, ri.isChase, ri.si.isTimeShift, rm.form, ri.isFmp4);
    ...
    public override void resetCommentFile()
    {
        ...
        else
        {
            ...
            try
            {
                if (bool.Parse(rm.cfg.get("IsgetComment")) && commentSW == null && !rfu.isPlayOnlyMode)
                {
                    ...
                    var fName = commentFileName == null
                        ? ri.recFolderFile[1]
                        : incrementRecFolderFile(commentFileName);
                    commentFileName = fName;
                    var _commentFileName =
                        util.getOkCommentFileName(rm.cfg, fName, ri.si.lvid, ri.si.isTimeShift, ri.isRtmp);
                    var isExists = File.Exists(_commentFileName);
            ...
            {
                util.debugWriteLine("on open wsc0 req send exception " + ee.Message + ee.Source + ee.StackTrace +
                                    ee.TargetSite);
            }
            ...
            {
                util.debugWriteLine("on open wsc1 req send exception " + ee.Message + ee.Source + ee.StackTrace +
                                    ee.TargetSite);
            }
        }
    }
    ...
    private void rtmpRecord(string websocketMsg, string quality)
    {
        ...
        {
            rr = new RtmpRecorder(ri.si.lvid, container, rm, rfu, !ri.isRtmp, ri.recFolderFile, this,
                ri.si.openTime);
            Task.Run(() =>
            {
    ...
    private bool isChaseStream(string msg)
    {
        ...
        if (res != null)
        {
            ...
            {
                var duration = (int)double.Parse(_duration, NumberStyles.Float);
    ...
    public override void chaseCommentSum()
    {
        ...
        util.debugWriteLine("chasecomment sum gotts " +
                            (gotTsCommentList.Length == 0 ? "no" : gotTsCommentList[0]));
        util.debugWriteLine("chasecomment sum chasecombuf " +
                            (chaseCommentBuf.Count == 0 ? "no" : chaseCommentBuf[0]));
    ...
    */
    internal override bool getStatistics(string lvid, CookieContainer cc, out string visit, out string comment)
    {
        visit = "0";
        comment = "0";
        ...
        if (res == null) return false;
        var _visit = util.getRegGroup(res,
            "statistics&quot;:{&quot;watchCount&quot;:(\\d+),&quot;commentCount&quot;:\\d+}");
        var _comment = util.getRegGroup(res,
            "statistics&quot;:{&quot;watchCount&quot;:\\d+,&quot;commentCount&quot;:(\\d+)}");
    ...
    public override void setSync(int no, double second, string m3u8Url)
    {
        try
        {
            if (ri.si.isTimeShift)
            {
                ...
                util.debugWriteLine("setSync ts " + no + " " + ri.si._openTime + " " + ri.si.openTime + " " +
                                    ri.si.vposBaseTime + " " + second);
            ...
            util.debugWriteLine("setSync " + no + " " + second + " " + m3u8Url + " " + ri.si._openTime + " " +
                                ri.si.openTime + " " + ri.si.vposBaseTime);
    ...
    private void checkMissingComment()
    {
        ...
        try
        {
            ...
            */
            ...
            util.debugWriteLine("last comment ind " + ind + " " + gotTsCommentList.Length + " " +
                                lastSaveComments.Count);
            if (ind == -1) return;
