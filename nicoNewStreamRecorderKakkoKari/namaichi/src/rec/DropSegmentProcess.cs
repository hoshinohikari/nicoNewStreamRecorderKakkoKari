...
{
    private readonly int lastSegmentNo;
    private readonly DateTime lastWroteSegmentDt;
    private readonly Record rec;
    private readonly string recFolderFileOrigin;
    ...
    private ChaseHokan chaseHokan;
    private numTaskInfo nti;

    public DropSegmentProcess(DateTime _lastWroteSegmentDt, int _lastSegmentNo, Record rec,
        string recFolderFileOrigin, RecordFromUrl rfu, RecordingManager rm, RecordInfo ri)
    {
//			this.nti = s;
        lastWroteSegmentDt = _lastWroteSegmentDt;
        lastSegmentNo = _lastSegmentNo;
        this.rec = rec;
    ...
    public bool start(numTaskInfo nti)
    {
        try
        {
            ...
            rec.addDebugBuf("drop segment process s.dt " + nti.dt + " lastwrote " + lastWroteSegmentDt + " now " +
                            DateTime.Now + " nti.no " + nti.no + " nti.originNo " + nti.originNo +
            ...
            if (rfu.subGotNumTaskInfo != null)
            {
                ...
                if (rfu.subGotNumTaskInfo.Count > 0)
                    rec.addDebugBuf("drop segment process subList min " + subL[0].dt + " " + subL[0].no + " max " +
                                    subL[subL.Count - 1].dt + " " + subL[subL.Count - 1].no);
            }
            ...
            var msg = lastWroteSegmentDt + "ぐらいから" + nti.dt + "ぐらいまでの動画データが取得できませんでした。(" + dropTime + "秒)";
            var hokanMsg = fName == null
                ? "補完設定がされていませんでした。"
                :
            ...
            if (fName != null)
            {
                ...
                chaseHokan = new ChaseHokan(nti, lastSegmentNo, fName, rfu.lvid, rm, ri,
    ...
    private string[] writeNukeSegment()
    {
        try
        {
            ...
            */
            if (!bool.Parse(rm.cfg.get("IsHokan"))) return null;
    ...
    */
    private string[] getOkFileName()
    {
        ...
        for (var i = 0; i < 1000; i++)
        {
            ...
            var name0 = (name + i).Replace('/', '\\');

            var isExists = false;
            foreach (var f in files)
                if (f.IndexOf(name0) != -1)
                    isExists = true;
            ...
            if (!isExists)
                //return name;
                return new[] { name0, name + "_0h0m0s_" + i };
        }
    ...
    public void writeRemaining()
    {
        try
        {
            rec.addDebugBuf("write remaining dropSegment process lastWrote dt " + lastWroteSegmentDt +
                            " lastSegmentNo " + lastSegmentNo + " now " + DateTime.Now + " sub got list len " +
                            rfu.subGotNumTaskInfo.Count);
            ...
            var msg = lastWroteSegmentDt + "ぐらいから" + "最終セグメントまでの動画データが取得できませんでした。(" +
                      rfu.subGotNumTaskInfo.Count * second + ")";
            var hokanMsg = fName == null ? "補完設定がされていませんでした。" :
                fName == "" ? "補完用データがありませんでした。" : "補完を試みました。" + fName;
