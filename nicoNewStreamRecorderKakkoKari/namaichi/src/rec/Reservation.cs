...
public class Reservation
{
    ...
    private readonly string lv;
    private readonly string reservationsUrl = "https://live.nicovideo.jp/api/timeshift.reservations";

    private readonly string useUrl = "https://live.nicovideo.jp/api/timeshift.ticket.use";
    ...
    public string reserve()
    {
        ...
        var url = "";
        if (getMode == "watchNum")
            url = "https://live.nicovideo.jp/api/watchingreservation?mode=watch_num&vid=" + id +
                  "&next_url=gate%2F" + lv + "&analytic=gate_modal_0_official_" + lv + "_closed";
        ...
        while (true)
        {
            ...
            if (wrRet.IndexOf("を削除して、新しくタイムシフト予約をします。") > -1) return "予約リストが一杯です。";
...
//				}
    ...
    private async Task<string> watchingReservation(string token, string id, string mode)
    {
    ...
    public string live2Reserve()
    {
        ...
        if (res.IndexOf("TICKET_NOT_FOUND") > -1)
        {
            ...
            if (res2.IndexOf("Timeshift_reservation was used") > -1)
                return "すでに視聴済みです。";
            if (res2.IndexOf("status\":200") > -1)
                return "ok";
            if (res2.IndexOf("Timeshift_reservation was expired") > -1)
                return "予約の期限切れでした。";
