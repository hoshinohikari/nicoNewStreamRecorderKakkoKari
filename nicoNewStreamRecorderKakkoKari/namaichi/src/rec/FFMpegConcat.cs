...
public class FFMpegConcat
{
    ...
    private readonly bool isFFmpeg = true;
...
//			util.debugWriteLine(command[command.Length - 2].Trim('\"'));
        ...
        */
        ...
        try
        {
            ...
            m3u8 = m3u8.Replace("\\", "/");
            var pipe = new NamedPipeServerStream(pipeName, PipeDirection.Out, 2, PipeTransmissionMode.Message,
                PipeOptions.None, 10000, 10000);
...
//				System.Threading.Thread.Sleep(3000);
    ...
    public void stopRecording()
    {
        ...
        if (isFFmpeg)
        {
            ...
            {
                if (!(ee is ObjectDisposedException))
                    util.debugWriteLine(ee.Message + ee.StackTrace);
                try
                {
                    process.Kill();
        ...
        while (!process.HasExited) Thread.Sleep(200);
