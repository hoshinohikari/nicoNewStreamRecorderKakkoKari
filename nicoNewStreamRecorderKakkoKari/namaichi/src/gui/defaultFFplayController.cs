...
public partial class defaultFFplayController : Form
{
    ...
    public bool isMute;
    private bool onMuteButtonArea;
    public Process process;
    public int volume;

    public defaultFFplayController(config.Config config, Process process, Player player)
    {
        this.config = config;
        ...
        try
        {
            ...
            Location = new Point(int.Parse(config.get("defaultControllerX")),
...
//				Top = int.Parse(config.get("Y"));
    ...
    {
        ControlPaint.DrawBorder(e.Graphics, timeLabel.ClientRectangle, Color.FromArgb(64, 64, 64),
    ...
    private void muteButton_MouseLeave(object sender, EventArgs _e)
    {
    ...
    private void reconnectButton_MouseEnter(object sender, EventArgs e)
    {
    ...
    private void reconnectButton_MouseLeave(object sender, EventArgs _e)
    {
        var e = PointToClient(Cursor.Position);
...
//				util.debugWriteLine("volume set " + v);
            while (player == null || player.pipeWriter == null) Thread.Sleep(1000);
    ...
    private bool kakuninClose()
    {
        try
        {
            util.debugWriteLine("defaultControllerX  " + Location.X + " defaultControllerX " + Location.Y +
                                " volume " + volumeBarArea.Width);
