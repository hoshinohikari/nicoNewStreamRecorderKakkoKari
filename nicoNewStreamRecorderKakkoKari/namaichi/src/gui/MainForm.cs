...
public partial class MainForm : Form
{
    ...
    public RecordingManager rec;
    public string recEndProcess;
    ...
    public MainForm(string[] args)
    {
        ...
        this.args = args;
        rec = new RecordingManager(this, config);
    ...
    protected override bool ShowWithoutActivation => args.Length > 0 && bool.Parse(config.get("Isminimized"));
    ...
    public void setRecordState(string t, string titleT = null, int per = 0)
    {
        ...
        formAction(() =>
        {
            try
            {
                ...
                if (rec.isTitleBarInfo) Text = titleT == null ? t : titleT;
                if (per < 100)
                {
        ...
        string endTime, bool isReservation)
    {
        ...
        formAction(() =>
        {
            try
            {
                ...
                hostLabel.Text = host;
                if (hostUrl != null) hostLabel.Links.Add(0, hostLabel.Text.Length, hostUrl);
                //				        	    hostLabel.LinkArea = new LinkArea(0, hostLabel.Text.Length);
                communityLabel.Text = group;
                if (groupUrl != null) communityLabel.Links.Add(0, communityLabel.Text.Length, groupUrl);
    ...
    public void setSamune(string url)
    {
        ...
        formAction(() =>
        {
            try
            {
                ...
                if (bool.Parse(config.get("IstitlebarSamune"))) Icon = icon;
            }
            catch (Exception e)
            {
                util.debugWriteLine(e.Message + " " + e.StackTrace + " " + e.Source + " " + e.TargetSite);
...
//       	player.addComment(time, comment, userId, score, color);
    ...
    private void copyUrlMenu_Clicked(object sender, EventArgs e)
    {
        try
        {
    ...
    private void copyCommunityUrlMenu_Clicked(object sender, EventArgs e)
    {
        try
        {
            if (communityLabel.Links.Count > 0 && communityLabel.Links[0].Length != 0)
            {
                var url = (string)communityLabel.Links[0].LinkData;
                Clipboard.SetText(url);
            }
        }
        catch (Exception ee)
    ...
    private void copyHost_UrlMenu_Clicked(object sender, EventArgs e)
    {
        try
        {
    ...
    private void openRecFolderMenu_Clicked(object sender, EventArgs e)
    {
        openRecFolder();
    }
...
//			if (kakuninClose()) Close();;
    ...
    private bool kakuninClose()
    {
        if (rec.rfu != null && bool.Parse(config.get("IsConfirmCloseMsgBox")))
        {
            ...
            var res = util.showMessageBoxCenterForm(this, _m + "中ですが終了しますか？", "確認", MessageBoxButtons.YesNo,
        ...
        try
        {
            util.debugWriteLine("width " + Width + " height " + Height + " restore width " + RestoreBounds.Width +
...
//			titleLabel.LinkArea = new LinkArea(0,0);
    ...
    private void PlayerBtnClick(object sender, EventArgs e)
    {
        player.play();
    }
    ...
    private void mainForm_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var desc = FileVersionInfo.GetVersionInfo(util.getJarPath()[0] + "/websocket4net.dll");
        ...
        try
        {
            ...
            {
                //dll
                var task = Task.Factory.StartNew(() => { util.dllCheck(this); });
            }
        }
        catch (Exception ee)
        {
        ...
        Task.Run(() =>
        {
            while (true)
                try
                {
                    ...
                    if (a == "stop end")
                    {
                        if (rec.rfu != null) rec.stopRecording(rec.rfu.isPlayOnlyMode);
                        while (rec.recordRunningList.Count > 0) Thread.Sleep(1000);
                        Close();
                    }
                }
                catch (Exception e)
    ...
    private bool isFullAccessDirectory()
    {
        //return true;
        /*
        try {
        ...
        */
        ...
        for (var i = 0; i < 1; i++)
            try
            {
                cfg.AppSettings.Settings["browserNum2"].Value =
                    (1 - int.Parse(cfg.AppSettings.Settings["browserNum2"].Value)).ToString();
                cfg.Save();
                return true;
            }
            catch (Exception e)
    ...
    private void MiniBtnClick(object sender, EventArgs e)
    {
        if (originalSize == Size.Empty)
            changeSize(true);
        else
    ...
    private void changeSize(bool isMini)
    {
        try
        {
            ...
            titleLabel.Size = isMini
                ? new Size(streamInfoGroupBox.Width - 107, 12)
                : new Size(streamInfoGroupBox.Width - 93, 23);
            ...
            communityLabel.Size =
                isMini ? new Size(streamInfoGroupBox.Width - 107, 12) : new Size(streamInfoGroupBox.Width - 93, 23);
            //hostLabel.Size = isMini ? new Size(streamInfoGroupBox.Width - 10, 12) : new Size(streamInfoGroupBox.Width - 93, 23);
            hostLabel.Size = isMini
                ? new Size(streamInfoGroupBox.Width - 107, 12)
                : new Size(streamInfoGroupBox.Width - 93, 23);
            ...
            reservationBox.Location =
                isMini ? new Point(titleLabel.Location.X + 135, 13) : new Point(streamInfoGroupBox.Width - 21, 23);
            setToolTip();
        }
        catch (Exception e)
        {
    ...
    private void LinkColorMenuItemClick(object sender, EventArgs e)
    {
        var d = new ColorDialog();
    ...
    private void OpenTourokuExeMenuClick(object sender, EventArgs e)
    {
        var jarpath = util.getJarPath();
        var path = jarpath[0] + "/録画登録ツール（仮.exe";
        try
        {
            if (!File.Exists(path))
            {
    ...
    {
        recBtn.Click -= recBtnAction;
        recBtn.MouseClick -= recBtnAction;
    ...
    private void RecEndMenuItemDropDownItemClicked(object sender, ToolStripItemClickedEventArgs e)
    {
    ...
    private void OpenNotifyIconMenuClick(object sender, EventArgs e)
    {
        activateForm();
    }

    private void CloseNotifyIconMenuClick(object sender, EventArgs e)
    {
        close();
    }
