...
public class Config
{
    public static Dictionary<int, string> qualityList = new()
    {
        { 0, "3Mbps(super_high)" },
    ...
    public Dictionary<string, string> argConfig = new();
    public string brokenCopyFile = null!;
    private Configuration cfg;
    public Dictionary<string, string> defaultConfig = null!;
    ...
    public Configuration getConfig()
    {
        for (var i = 0; i < 5; i++)
            ...
            catch (Exception e)
            {
                ...
                if (e.Message.IndexOf("レベルのデータ", StringComparison.Ordinal) > -1) break;
                Thread.Sleep(3000);
                //ルート レベルのデータが無効です。
            }
    ...
    {
        if (key.IndexOf("user_session", StringComparison.Ordinal) == -1 &&
            key.IndexOf("account", StringComparison.Ordinal) == -1)
    ...
    {
        foreach (var _l in l)
            if (_l.Key.IndexOf("user_session", StringComparison.Ordinal) == -1 &&
                _l.Key.IndexOf("account", StringComparison.Ordinal) == -1)
        ...
        for (var i = 0; i < 100; i++)
        {
            ...
            foreach (var _l in l)
                if (Array.IndexOf(keys, _l.Key) < 0)
    ...
    public string get(string key)
    {
        ...
        {
            util.debugWriteLine("config get exception " + key + " " + e.Message + e.Source + e.StackTrace +
                                e.TargetSite);
            return null!;
        }
...
//			{"isAllBrowserMode","true"},
            ...
            {
                "qualityList",
                "{\"0\":\"3Mbps(super_high)\",\"1\":\"2Mbps(high)\",\"2\":\"1Mbps(normal)\",\"3\":\"384kbps(low)\",\"4\":\"192kbps(super_low)\",\"5\":\"音声のみ(audio_high)\",\"6\":\"6Mbps(6Mbps1080p30fps)\",\"7\":\"8Mbps(8Mbps1080p60fps)\",\"8\":\"4Mbps(4Mbps720p60fps)\",\"9\":\"音声のみ(audio_only)\"}"
            },
...
//			{"IsDefaultEngine","true"},
            ...
            { "rokugaTourokuWidth", "1300" },
            { "rokugaTourokuHeight", "700" },
        ...
        try
        {
            ...
            foreach (var k in cfg.AppSettings.Settings.AllKeys) buf.Add(k, cfg.AppSettings.Settings[k].Value);
    ...
    public void saveFromForm(Dictionary<string, string> formData)
    {
        ...
        foreach (var k in formData.Keys) cfg.AppSettings.Settings[k].Value = formData[k];
