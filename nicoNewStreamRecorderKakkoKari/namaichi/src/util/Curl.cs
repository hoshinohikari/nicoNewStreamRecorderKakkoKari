...
public class Curl
{
    private readonly IntPtr curlM = IntPtr.Zero;
    ...
    public byte[] getBytes(string url, Dictionary<string, string> headers, CurlHttpVersion httpVer,
        string method = "GET", string postData = "", bool isAddHeader = false)
    {
        try
        {
    ...
    public string getStr(string url, Dictionary<string, string> headers, CurlHttpVersion httpVer,
        string method = "GET", string postData = "", bool isAddHeader = false)
    {
        try
        {
    ...
    public List<KeyValuePair<string, byte[]>> get(List<string> urlList, Dictionary<string, string> headers,
        CurlHttpVersion httpVer, string method = "GET", string postData = "", bool isAddHeader = false)
    {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (msg.msg == CURLMSG.CURLMSG_DONE)
                    {
                        ...
                        var dHeader = new byte[_chunk.sizeHeader];
                        Marshal.Copy(_chunk.memoryHeader, dHeader, 0, _chunk.sizeHeader - 0);

                        var d = new byte[_chunk.size + (isAddHeader ? _chunk.sizeHeader : 0)];
                        easyD.res = d;
    ...
    */
    ...
    private class easyData
    {
        ...
        public bool setEasy(string url, Dictionary<string, string> headers, CurlHttpVersion httpVer, string method,
            IntPtr postData)
        {
            ...
            curl_easy_setopt(easyPtr, CURLoption.CURLOPT_TIMEOUT, 20);
            if (method == "POST") curl_easy_setopt(easyPtr, CURLoption.CURLOPT_POSTFIELDS, postData);
