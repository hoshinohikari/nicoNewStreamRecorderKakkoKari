...
internal class util
{
    ...
    public static WebProxy httpProxy;
    public static HttpConnectProxy wsProxy;

    public static string userAgent =
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.5005.63 Safari/537.36";

    public static bool isLogFile;
    public static List<string> debugWriteBuf = new();
    private static Form messageBoxOwnerForm;
...
//		util.consoleWrite(p);
    ...
    public static string[] getJarPath()
    {
        ...
        if (isTestMode) return new[] { "C:\\Users\\pc\\desktop", "util", "exe" };

        //			string f=Environment.GetCommandLineArgs()[0];
        var f = Assembly.GetExecutingAssembly().Location;
//			util.debugWriteLine(Environment.GetCommandLineArgs().Length);
        f = Path.GetFileName(f);

        var withoutKakutyousi = f.IndexOf(".") < 0 ? f : getRegGroup(f, "^(.*)\\.");
        var kakutyousi = f.IndexOf(".") < 0 ? null : getRegGroup(f, "^.*\\.(.*)");

        debugWriteLine(getPath() + " " + withoutKakutyousi + " " + kakutyousi);
        //0-dir 1-withoutKakutyousi 2-kakutyousi
        return new[] { getPath(), withoutKakutyousi, kakutyousi };
    }
        ...
        long _openTime, bool isRtmp, bool isFmp4)
    {
        ...
        var _isTimeShift = isTimeShift;
        var segmentSaveType = cfg.get("segmentSaveType");
        var ext = isFmp4 ? ".mp4" : ".ts";
        if (tsConfig != null && tsConfig.lastFileName != null)
        {
            name = getRegGroup(tsConfig.lastFileName, ".+[\\\\/](.+)_ts_\\d+h\\d+m\\d+s");
            dirPath = getRegGroup(tsConfig.lastFileName, "(.+[\\\\/])");
            debugWriteLine("getRecFolderFilePath tsconfig.lastFileName " + name + " " + dirPath);
        }
        else
        {
            ...
            var _dirPath = cfg.get("IsdefaultRecordDir") == "true" ? jarpath[0] + "\\rec" : cfg.get("recordDir");
            dirPath = _dirPath;
            ...
            if (name.Length + dirPath.Length > 234) return new[] { null, name + " " + dirPath, null };
        ...
        {
            var fName = dirPath + "/" + name + "_" + (_isTimeShift ? "ts" : "") + i;
            var originName = dirPath + "/" + name;
            debugWriteLine(dirPath + " " + fName);
            ...
            {
                var _existFile = util.existFile(files, "_ts(_\\d+h\\d+m\\d+s_)*" + i + "", name);
                var _existDt = util.existFile(files, "_ts_(\\d+h\\d+m\\d+s)_" + i + ext, name);
                var reg = "_ts_(\\d+h\\d+m\\d+s)_" + i + ext;
                if (_existDt != null)
                {
                    existDt = getRegGroup(_existDt, "(\\d+h\\d+m\\d+s)");
                ...
                if (tsConfig.isContinueConcat)
                {
                    if (i == 0 || existDt == null)
                    {
                        var firstFile = dirPath + "/" + name + "_ts_0h0m0s_" + i;
...
//							existFile = existFile.Substring(0, existFile.Length - 3);
    ...
    private static string getSubFolderName(string host, string group, string title, string lvId, string communityNum,
        string userId, Config cfg)
    {
        var n = cfg.get("subFolderNameType");
        if (n == null) n = "1";
        if (n == "1") return host;
        if (n == "2") return userId;
        if (n == "3") return userId + "_" + host + "";
        if (n == "4") return group;
        if (n == "5") return communityNum;
        if (n == "6") return communityNum + "_" + group + "";
        if (n == "7") return communityNum + "_" + host + "";
        if (n == "8") return host + "_" + communityNum + "";
        return host;
    }

    public static string getFileName(string host, string group, string title, string lvId, string communityNum,
        Config cfg, long _openTime)
    {
        var n = cfg.get("fileNameType");
        //var _hiduke = DateTime.Now;
        var _hiduke = getUnixToDatetime(_openTime);
        var month = _hiduke.Month < 10 ? "0" + _hiduke.Month : _hiduke.Month.ToString();
        var day = _hiduke.Day < 10 ? "0" + _hiduke.Day : _hiduke.Day.ToString();
        var hiduke = _hiduke.Year + "年" + month + "月" + day + "日";
        if (n == null) n = "1";
        if (n == "1") return host + "_" + communityNum + "(" + group + ")_" + lvId + "(" + title + ")";
        if (n == "2") return communityNum + "(" + group + ")_" + host + "_" + lvId + "(" + title + ")";

        if (n == "3") return lvId + "(" + title + ")_" + host + "_" + communityNum + "(" + group + ")";
        if (n == "4") return host + "_" + group + "(" + communityNum + ")_" + title + "(" + lvId + ")";
        if (n == "5") return group + "(" + communityNum + ")_" + host + "_" + title + "(" + lvId + ")";
        if (n == "6") return title + "(" + lvId + ")_" + host + "_" + group + "(" + communityNum + ")";
        if (n == "7") return hiduke + "_" + host + "_" + group + "(" + communityNum + ")_" + title + "(" + lvId + ")";
        if (n == "8") return hiduke + "_" + group + "(" + communityNum + ")_" + host + "_" + title + "(" + lvId + ")";
        if (n == "9") return hiduke + "_" + title + "(" + lvId + ")_" + host + "_" + group + "(" + communityNum + ")";
        if (n == "10")
            return getDokujiSetteiFileName(host, group, title, lvId, communityNum, cfg.get("filenameformat"), _hiduke);
        return host + "_" + communityNum + "(" + group + ")_" + lvId + "(" + title + ")";
    }

    public static string getDokujiSetteiFileName(string host, string group, string title, string lvId,
        string communityNum, string format, DateTime _openTime)
    {
        var type = format;
        if (type == null) return "";
        //var dt = DateTime.Now;
        var dt = _openTime;
        var yearBuf = "0000" + dt.Year;
        var year2 = yearBuf.Substring(yearBuf.Length - 2);
        var year4 = yearBuf.Substring(yearBuf.Length - 4);
        var monthBuf = "00" + dt.Month;
        var month = monthBuf.Substring(monthBuf.Length - 2);
        var dayBuf = "00" + dt.Day;
    ...
    public static string getOkCommentFileName(Config cfg, string fName, string lvid, bool isTimeShift, bool isRtmp)
    {
        ...
        if (cfg.get("segmentSaveType") == "0" || engineMode != "0" || isRtmp)
        {
            ...
            debugWriteLine("comment file path " + fName + kakutyousi);
            return fName + kakutyousi;
        }

        var name = getRegGroup(fName, ".+/(.+)");
        if (fName.Length + name.Length > 245) name = lvid;
        debugWriteLine("comment file path " + fName + "/" + name + kakutyousi);
        ...
        string userId, Config cfg, long _openTime, bool isFmp4)
    {
        ...
        debugWriteLine("getLastTimeshiftFileName dirPath " + dirPath + " sfn " + sfn);
        ...
        debugWriteLine("getLastTimeshiftFileName name " + name);
        ...
        if (name.Length + dirPath.Length > 234)
        {
            ...
            if (name.Length + dirPath.Length > 234 && sfn != null)
            {
                ...
                {
                    debugWriteLine("getLastTS FN too long not exist dir path " + dirPath);
                    return null;
                }
            }
        }
        ...
        if (name.Length + dirPath.Length > 234)
        {
            debugWriteLine("too long " + name + " " + dirPath + " " + name.Length + " " + dirPath.Length);
            return null;
        }

        if (!Directory.Exists(dirPath))
        {
            debugWriteLine("no exists " + dirPath);
            return null;
        }

        debugWriteLine("getLast TS FN 00");

        string existFile = null;
        var files = Directory.GetFiles(dirPath);
        for (var i = 0; i < 1000; i++)
        {
            var fName = dirPath + "/" + name + "_" + "ts" + i;

            if (segmentSaveType == "0")
            {
                //util.existFile(dirPath, name + "_ts_\\d+h\\d+m\\d+s_" + i.ToString());
                var _existFile = util.existFile(files, "_ts_(\\d+h\\d+m\\d+s)_" + i + ext, name);
                debugWriteLine("getLastTimeshiftFileName existfile " + _existFile);
                if (_existFile != null)
                {
                    existFile = _existFile;
                    continue;
            ...
            debugWriteLine("getLastTimeshiftFileName dirpath " + dirPath + " fname " + fName);

            if (i == 0)
            {
                debugWriteLine("last timeshift file " + dirPath + "/" + name + "_" + "ts" + (i - 0));
                return null;
//			} else util.debugWriteLine("last timeshift file " + dirPath + "/" + name + "_" + "ts" + (i - 1).ToString());
            }

            debugWriteLine("last timeshift file " + existFile);
...
//			string[] ret = {dirPath, dirPath + "/" + name + "_" + "ts" + (i - 1).ToString()};
    ...
    public static string[] getLastTimeShiftFileTime(string lastFile, string segmentSaveType, bool isFmp4)
    {
        ...
        else
        {
            ...
            foreach (var f in Directory.GetFiles(lastFile))
            {
                ...
                var name = getRegGroup(f, ".+\\\\(.+)");
                if (name == null) continue;
                if (getRegGroup(name, "(\\d+h\\d+m\\d+s)") == null) continue;
                ;
                var _k = getRegGroup(name, "(\\d+)");
                if (_k == null) continue;
                ss.Add(f);
        ...
        var _name = getRegGroup(fname, ".+\\\\(.+)");
        var h = getRegGroup(_name, "(\\d+)h");
        var m = getRegGroup(_name, "(\\d+)m");
        var s = getRegGroup(_name, "(\\d+)s");
    ...
    private static bool isExistAllExt(string fName, string folder)
    {
        ...
        {
            var reg = new Regex(
                Regex.Escape(fName.Replace("\\", "/")).Replace("\\{w}", "\\d*").Replace("\\{c}", "\\d*") + e);
            foreach (var f in files)
                if (reg.Match(f.Replace("\\", "/")).Success)
                    return true;
    ...
    private static bool isExistDirectory(string fName, string dirPath)
    {
        ...
        {
            var reg = new Regex(
                Regex.Escape(fName.Replace("\\", "/")).Replace("\\{w}", "\\d*").Replace("\\{c}", "\\d*"));
    ...
    public static string incrementRecFolderFile(string recFolderFile)
    {
        if (recFolderFile.EndsWith("xml") || recFolderFile.EndsWith("json"))
        {
            ...
            for (var i = int.Parse(m.Groups[1].Value); i < 10000; i++)
            {
                var _new = i + 1 + "." + m.Groups[2];
    ...
    public static string getPageSource(string _url, CookieContainer container = null, string referer = null,
        bool isFirstLog = true, int timeoutMs = 0, string userAgent = null, bool isGetErrorPage = false)
    {
        debugWriteLine("access__ getpage " + _url);
        if (_url == null)
        {
            debugWriteLine("getPageSource url null");
            return null;
        }
...
//		util.debugWriteLine("getpage 02");
        for (var i = 0; i < 1; i++)
            ...
            {
                Task.Run(() => { debugWriteLine("getpage error " + _url + e.Message + e.StackTrace); });
    ...
    {
        debugWriteLine("access__ getFilebytes " + url);
//		var a = container.GetCookieHeader(new Uri(_url));
        ...
        for (var i = 0; i < 1; i++)
            ...
            {
                Task.Run(() => { debugWriteLine("getfile error " + url + e.Message + e.StackTrace); });
                //				System.Threading.Thread.Sleep(3000);
            }
    ...
    public static HttpWebResponse sendRequest(string url, Dictionary<string, string> headers, byte[] content,
        string method, CookieContainer cc = null)
    {
        try
        {
            var req = (HttpWebRequest)WebRequest.Create(url);
        ...
        {
            debugWriteLine(ee.Data + ee.Message + ee.Source + ee.StackTrace + ee.Status);
            try
            {
            ...
            {
                debugWriteLine(eee.Message + eee.Source + eee.StackTrace);
                return null;
            }
        }
        catch (Exception ee)
    ...
    public static bool isEndedProgram(string lvid, CookieContainer container, bool isSub)
    {
        ...
        var res = getPageSource(url, container);
        debugWriteLine("isendedprogram url " + url + " res==null " + (res == null) + getMainSubStr(isSub, true));
//			util.debugWriteLine("isendedprogram res " + res + util.getMainSubStr(isSub, true));
        ...
        debugWriteLine("is ended program " + isEnd + getMainSubStr(isSub, true));
        return isEnd;
    }
...
//		var files = Directory.GetFiles(dirPath);
        ...
        foreach (var f in files)
        {
            ...
            var _reg = getRegGroup(_f.Substring(nameLen), reg);
...
//		var dotSecond = ((int)((seconds % 1) * 10)).ToString("0");
    ...
    public static int getPageType(string res)
    {
        ...
        var data = getRegGroup(res, "<script id=\"embedded-data\" data-props=\"([\\d\\D]+?)</script>");
        var status = data == null ? null : getRegGroup(data, "&quot;status&quot;:&quot;(.+?)&quot;");
        var opentime = data != null ? getRegGroup(data, "&quot;openTime&quot;:(\\d+)") : null;
        var serverTime = data != null ? getRegGroup(data, "&quot;serverTime&quot;:(\\d+)") : null;
        if (res.IndexOf("<!doctype html>") > -1 && data != null && status == "ON_AIR" &&
            data.IndexOf("webSocketUrl&quot;:&quot;ws") > -1) return 0;
        if (res.IndexOf("<!doctype html>") > -1 && data != null && status == "ENDED" &&
            data.IndexOf("webSocketUrl&quot;:&quot;ws") > -1) return 7;
        if (getRegGroup(res, "(混雑中ですが、プレミアム会員の方は優先して入場ができます)") != null ||
            getRegGroup(res, "(ただいま、満員のため入場できません)") != null) return 1;
        //		else if (util.getRegGroup(res, "<div id=\"comment_arealv\\d+\">[^<]+この番組は\\d+/\\d+/\\d+\\(.\\) \\d+:\\d+に終了いたしました。<br>") != null) return 2;
        if (res.IndexOf(" onclick=\"Nicolive.ProductSerial") > -1) return 8;
        ...
        if (res.IndexOf("isFollowerOnly&quot;:true") > -1 && res.IndexOf("isJoined&quot;:false") > -1 &&
            res.IndexOf("[&quot;noTimeshiftProgram") == -1) return 4;

        if (status == "ENDED" && res.IndexOf("rejectedReasons&quot;:[&quot;notHaveTimeshiftTicket") > -1) return 9;
        if (status == "ENDED" && res.IndexOf("rejectedReasons&quot;:[&quot;notUseTimeshiftTicket") > -1) return 10;
        if (data != null && data.IndexOf("webSocketUrl&quot;:&quot;ws") == -1 &&
            status == "ENDED") return 2;
        if (res.IndexOf("rejectedReasons&quot;:[&quot;notHavePayTicket") > -1) return 11;
        ...
        if (getRegGroup(res, "(<archive>1</archive>)") != null) return 3;
        //else if (util.getRegGroup(res, "(チャンネル会員限定番組です。<br>)") != null) return 4;
        if (getRegGroup(res, "(<h3>【会場のご案内】</h3>)") != null) return 6;
        if (getRegGroup(res, "(この番組は放送者により削除されました。<br />|削除された可能性があります。<br />)") != null) return 2;
        if (res.IndexOf("rejectedReasons&quot;:[&quot;notActivatedBySerial&quot;]") > -1) return 8;
        if (res.IndexOf("content=\"https://jk.nicovideo.jp/\"") > -1) return 12;
        if (opentime != null && serverTime != null &&
            long.Parse(opentime) - long.Parse(serverTime) / 1000 > 15) return 13;
        return 5;
        //}
        //return 5;
    }
...
//		var res = getPlayerStatusRes;
        ...
        if (res.IndexOf("<code>require_community_member</code>") > -1)
            return 4;
        if (res.IndexOf("<code>closed</code>") > -1)
            return 2;
        if (res.IndexOf("<code>comingsoon</code>") > -1)
            return 5;
        if (res.IndexOf("<code>notfound</code>") > -1)
            return 5;
        if (res.IndexOf("<code>deletedbyuser</code>") > -1)
            return 2;
        if (res.IndexOf("<code>deletedbyvisor</code>") > -1)
            return 2;
        if (res.IndexOf("<code>violated</code>") > -1)
            return 2;
        if (res.IndexOf("<code>usertimeshift</code>") > -1)
            return 2;
        if (res.IndexOf("<code>tsarchive</code>") > -1)
            return 5;
        if (res.IndexOf("<code>unknown_error</code>") > -1)
            return 5;
        if (res.IndexOf("<code>timeshift_ticket_exhaust</code>") > -1)
            return 2;
        if (res.IndexOf("<code>timeshiftfull</code>") > -1)
            return 1;
        if (res.IndexOf("<code>maintenance</code>") > -1)
            return 5;
        if (res.IndexOf("<code>noauth</code>") > -1)
            return 5;
        if (res.IndexOf("<code>full</code>") > -1)
            return 1;
        if (res.IndexOf("<code>block_now_count_overflow</code>") > -1)
            return 5;
        if (res.IndexOf("<code>premium_only</code>") > -1)
            return 5;
        if (res.IndexOf("<code>selected-country</code>") > -1)
            return 5;
        if (res.IndexOf("<code>notlogin</code>") > -1) return 8;
    ...
    {
        using (var f = new FileStream(name, FileMode.Append))
        using (var w = new StreamWriter(f))
        {
            ...
            {
                debugWriteLine(e.Message + " " + e.StackTrace + " " + e.TargetSite);
            }
        }
//		w.Close();
//		f.Close();
    ...
    public static string getOkSJisOut(string s, string replaceStr = null)
    {
        try
        {
            ...
            var a = Encoding.GetEncoding("shift_jis");
        ...
        {
            debugWriteLine(e.Message + e.Source + e.StackTrace + e.TargetSite);
            return s;
        }
    }
...
#else
            if (isLogFile) Console.WriteLine(dt + " " + str);
...
//			util.debugWriteLine(e.Message + " " + e.StackTrace + " " + e.TargetSite + " " + e.Source);
            Debug.WriteLine(e.Message + " " + e.StackTrace + " " + e.TargetSite + " " + e.Source);
        }
    }
    ...
    */
    public static void showException(Exception eo, bool isMessageBox = true)
    {
        ...
        if (isMessageBox && isLogFile)
            if (frameCount > 150)
            {
                showMessageBoxCenterForm(null, "framecount stack", frameCount + " " + Program.arg + " " + DateTime.Now);
                return;
            }
#else
#endif
        ...
        debugWriteLine("exception stacktrace framecount " + frameCount);
        debugWriteLine("show exception eo " + eo);
        if (eo == null) return;

        debugWriteLine("0 message exception " + eo.Message + "\nsource " +
                       eo.Source + "\nstacktrace " + eo.StackTrace +
                       "\n targetsite " + eo.TargetSite + "\n\n");

        var _eo = eo.GetBaseException();
        debugWriteLine("eo " + _eo);
        if (_eo != null)
            debugWriteLine("1 message exception " + _eo.Message + "\nsource " +
        ...
        debugWriteLine("eo " + _eo);
        if (_eo != null)
            debugWriteLine("2 message exception " + _eo.Message + "\nsource " +
                           _eo.Source + "\nstacktrace " + _eo.StackTrace +
                           "\n targetsite " + _eo.TargetSite);

#if DEBUG
        if (isMessageBox && isLogFile)
            showMessageBoxCenterForm(null, "error " + eo.Message, "error " + Program.arg);
#else
#endif
    }
    ...
    public static void setLog(Config config, string lv)
    {
        if (bool.Parse(config.get("IsLogFile")))
        {
            ...
            var logPath = getJarPath()[0] + "/" + name + ".txt";
            ...
            {
                debugWriteLine(e.Message + " " + e.StackTrace + " " + e.Source + " " + e.TargetSite);
            }
    ...
    {
        debugWriteLine("releasekey " + releaseKey);
    ...
    public static void soundEnd(Config cfg, MainForm form)
    {
        try
        {
            var path = bool.Parse(cfg.get("IsSoundDefault"))
                ? getJarPath()[0] + "/Sound/se_soc02.wav"
                : cfg.get("soundPath");
            if (path == "") path = getJarPath()[0] + "/Sound/se_soc02.wav";

            var volume = float.Parse(cfg.get("soundVolume")) / 100;
            if (volume < 0) volume = 0;
            if (volume > 1) volume = 1;
            debugWriteLine("sound path " + path + " volume " + volume);
    ...
    public static void playSoundMCI(string path, int volume, bool isAsync, MainForm form)
    {
        ...
        if (ret != 0)
        {
            ...
            var errMsg = new StringBuilder(1000);
            mciGetErrorString(ret, errMsg, 1000);
            debugWriteLine("mci err " + errMsg);
            form.addLogText("サウンドの再生中に問題が発生しました。ERROR:" + ret + ", メッセージ:" + errMsg + " パス:" + path + ", volume:" +
                            volume);
            return;
        }

        ret = mciSendStringForm("setaudio " + name + " volume to " + volume, null, 0, IntPtr.Zero, form);
    ...
    private static extern int SetThreadExecutionState(uint esFlags);
    ...
    public static void setThreadExecutionState()
    {
        try
        {
            ...
            debugWriteLine("setThreadExecutionState " + r);
        }
        catch (Exception e)
        {
            debugWriteLine(e.Message + e.Source + e.StackTrace + e.TargetSite);
        ...
        try
        {
            ...
            foreach (var n in dlls)
                if (!File.Exists(path + "/" + n))
                    msg += (msg == "" ? "" : ",") + n;
            if (msg != "")
                form.formAction(() => showMessageBoxCenterForm(form, path + "内に" + msg + "が見つかりませんでした"));
        }
        catch (Exception e)
        {
            debugWriteLine(e.Message + e.Source + e.StackTrace);
            form.addLogText(e.Message + e.Source + e.Source + e.StackTrace);
        }

        try
        {
            new Action(() => { debugWriteLine(WebSocketVersion.Rfc6455); }).Invoke();
        }
        catch (Exception e)
        {
            debugWriteLine(e.Message + e.Source + e.StackTrace);
    ...
    public static void setProxy(Config cfg, MainForm form = null)
    {
        ...
        try
        {
            ...
            wsProxy = new HttpConnectProxy(endpoint);
        }
        catch (Exception e)
        {
            debugWriteLine(e.Message + e.Source + e.StackTrace + e.TargetSite);
        ...
        try
        {
            httpProxy = new WebProxy(proxyAddress, int.Parse(proxyPort));
        }
        catch (Exception e)
        {
            debugWriteLine(e.Message + e.Source + e.StackTrace + e.TargetSite);
    ...
    public static bool loginCheck(CookieContainer cc, string url, string log = null)
    {
        try
        {
            ...
            var uid = getRegGroup(us.Value, "user_session_(.+?)_");
            if (uid == null)
            {
                if (log != null)
    ...
    public static string getMyName(CookieContainer cc, string us)
    {
        try
        {
            ...
            using (var sr = new StreamReader(st))
            {
                ...
                var n = getRegGroup(res, "\"nickname\":\"(.+?)\"");
                return n;
            }
        }
        catch (Exception e)
    ...
    public static void setFontSize(float size, Form form, bool isKeepSize, int baseHeight = -1)
    {
        try
        {
            ...
            {
                size = max;
                showMessageBoxCenterForm(form, "画面上に表示できなくなる可能性があるため、" + size + "に設定されます");
            }
            ...
            var controls = getChildControls(form);
    ...
    public static void shutdown(string recEndProcess, Form form)
    {
        try
        {
            ...
            if (recEndProcess == "OSをシャットダウンする")
            {
                var r = showMessageBoxCenterForm(form, "1分後にシャットアウトしてよろしいですか？", "確認", MessageBoxButtons.YesNo);
    ...
    public static string getAboutNumStr(string s)
    {
        ...
        if (s.Length >= 8) return s.Substring(0, s.Length - 7) + "千万";
        if (s.Length >= 5) return s.Substring(0, s.Length - 4) + "万";
        return s;
    }
    ...
    public static string getReplacedComment(string t, List<string[]> commentReplaceList)
    {
        ...
        try
        {
            foreach (var s in commentReplaceList) t = new Regex(s[0]).Replace(t, s[1]);
            return t;
        }
        catch (Exception e)
        {
    ...
    private static extern int SendMessage(IntPtr hWnd, int Msg, int wParam,
        [MarshalAs(UnmanagedType.LPTStr)] StringBuilder buff);
    ...
    private static IntPtr CBTProc(int nCode, IntPtr wParam, IntPtr lParam)
    {
        ...
        if (nCode == HCBT_ACTIVATE)
        {
            ...
            if (getWindowText(wParam) == "開く")
            {
                ...
                EnumChildWindows(wParam, enumWindowCallBack, IntPtr.Zero);
            }
    ...
    private static bool enumWindowCallBack(IntPtr hwnd, IntPtr lParam)
    {
        ...
        if (s.StartsWith("開く")) SetWindowText(hwnd, "選択");
    ...
    public static DialogResult showMessageBoxCenterForm(Form form, string text, string caption = "",
        MessageBoxButtons btn = MessageBoxButtons.OK, MessageBoxIcon icon = MessageBoxIcon.None)
    {
        if (form != null)
        {
            ...
            mBHook = SetWindowsHookEx(whCbt, CBTProc, hInstance, threadId);
        }
    ...
    private struct COPYDATASTRUCT
    {
        ...
        [MarshalAs(UnmanagedType.LPWStr)] public readonly string lpData;
    }
