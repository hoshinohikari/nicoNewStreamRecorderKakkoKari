...
public class ArgReader
{
    ...
    private void setArgConfig()
    {
        ...
        var values = config.defaultConfig.Values.ToList();
        var keys = config.defaultConfig.Keys.ToList();
        lowKeys.AddRange(new[]
        {
            "ts-start", "ts-end",
            //"ts-list", "ts-list-update", "ts-list-command", "ts-list-open", "ts-list-m3u8", 
            "ts-vpos-starttime", "ts-starttime-comment", "ts-endtime-comment", "ts-starttime-open",
            "ts-endtime-open", "ts-endtime-delete-pos", "accountsetting"
        });
        foreach (var a in args)
            if (a.StartsWith("-"))
            {
    ...
    private bool isValidConf(string name, string val, List<string> lowKeys, List<string> defValues,
        out string setVal, out string setName, List<string> keys)
    {
        setVal = null;
        setName = null;
