...
{
    private readonly ChannelPlusRecorder cpr;
    private readonly Curl curl = new();
    private readonly StreamInfo si;
    private string auth;
    public CookieContainer cc = new();
    private string chPlusCookie;
    private DateTime lastTimeGotAuth = DateTime.MinValue;
    ...
    public string loginAuth()
    {
        ...
        try
        {
            var authUrl = "https://auth.sheeta.com/auth/realms/FCS00001/protocol/openid-connect/auth?client_id=" +
                          channelNum +
                          "&response_type=code&scope=openid&kc_idp_hint=niconico&redirect_uri=https://nicochannel.jp/" +
                          channelName + "/login";
            var r = curl.getStr(authUrl, Curl.getDefaultHeaders(), CurlHttpVersion.CURL_HTTP_VERSION_2TLS, "GET",
                "", true);
            ...
            */
            ...
            if (_location == null)
            {
                ...
                var f = new[]
                {
                    "random", "scope", "redirect_uri", "uri_token", "client_id", "csrf_token", "response_type",
                    "nonce", "state", "accepted"
                };
                foreach (var _f in f)
                {
                ...
                cpr.rm.form.formAction(
                    () => authPostRes = MessageBox.Show("ニコニコチャンネルプラスへのログインを認可しますか？(ニコニコのアカウント設定ページで取り消すことができます)",
            ...
            */
            ...
            var data = "{\"key_cloak_user\":{\"code\":\"" + code + "\",\"redirect_uri\":\"https://nicochannel.jp/" +
                       channelName + "/login\"},\"fanclub_site\":{\"id\":" + int.Parse(channelNum.Substring(3)) +
                       "}}";
    ...
    private void setHeaderToCookie(string host, string r, CookieContainer cc)
    {
    ...
    public bool isActiveAuth(string channelNum)
    {
        ...
        var r = curl.getStr("https://nfc-api.nicochannel.jp/fc/fanclub_sites/" + fNum + "/user_info", h,
            CurlHttpVersion.CURL_HTTP_VERSION_2TLS, "POST", "null", true);
        util.debugWriteLine("user info " + r);
        return r.IndexOf("\"fanclub_member\":null") == -1;
    }

    private string getNicoIdByChPlus()
    {
        try
        {
            ...
            //FCS00000
            var cn = int.Parse(_cn.Substring(3));
    ...
    private bool setCookie(string chNum, string chName)
    {
        /*
        ...
        */
        ...
        try
        {
            ...
            for (var i = 0; i < 5; i++)
                try
                {
                    var cgret = cg.getHtml5RecordCookie("https://live.nicovideo.jp/").Result;
                    //cgret.Wait();
