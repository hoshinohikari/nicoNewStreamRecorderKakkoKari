...
public class StreamInfo
{
    ...
    public bool isChannelPlus;
    public bool isChasable;
    public bool isReservation;
    public bool isRtmpOnlyPage;
    ...
    public void set(string res)
    {
        if (lvid.StartsWith("lv"))
        {
            ...
            data = isRtmpOnlyPage ? HttpUtility.UrlDecode(res) : HttpUtility.HtmlDecode(data);
            isReservation = lvid.StartsWith("lv")
                ? data.IndexOf("\"reservation\"") > -1
                : data.IndexOf("\"has_archived_files\":true") > -1;
    ...
    public bool getTimeInfo()
    {
        ...
        {
            var startTimeDtStr = util.getRegGroup(data, "\"live_started_at\":\"(.+?)\"");
            if (startTimeDtStr == null)
                startTimeDtStr = util.getRegGroup(data, "\"live_scheduled_start_at\":\"(.+?)\"");
    ...
    private string[] getHtml5RecFolderFileInfo(string data, string type, bool isRtmpOnlyPage)
    {
        ...
        if (lvid.StartsWith("lv"))
        {
            if (!isRtmpOnlyPage)
            {
                ...
                else
                {
                    ...
                    {
                        var isChannel = util.getRegGroup(data, "visualProviderType\":\"(channel)\",\"title\"") !=
                                        null;
                        ...
                        //			title = util.getRegGroup(res, "\\\\\"programHeader\\\\\":\\{\\\\\"thumbnailUrl.+?\\\\\"title\\\\\":\\\\\"(.*?)\\\\\"");
                        title = util.getRegGroup(data,
                            "visualProviderType\":\"(community|channel)\",\"title\":\"(.*?)\",", 2);
                        ...
                        userId = isChannel
                            ? "channel"
                            : util.getRegGroup(data,
                                "supplier\":{\"name\".+?pageUrl\":\"https*://www.nicovideo.jp/user/(\\d+?)\"");
                        ...
                        //			title = util.uniToOriginal(title);
                        util.debugWriteLine(host + " " + group + " " + title + " " + communityNum + " userid " +
                                            userId);
        ...
        else
        {
            ...
            if (_cn != null)
            {
                communityNum = _cn;
                var c = new Curl();
                var r = c.getStr("https://nfc-api.nicochannel.jp/fc/fanclub_sites/" + _cn + "/page_base_info",
                    Curl.getDefaultHeaders(), CurlHttpVersion.CURL_HTTP_VERSION_2TLS);
                if (r != null)
                {
        ...
        return new[] { host, group, title, lvid, communityNum, userId };
    }

    public StreamInfo clone()
    {
        return (StreamInfo)MemberwiseClone();
        /*
...
public class RecordInfo
{
    ...
    public bool isRealtimeChase;
    public bool isRtmp;
    public int pageType;

    public string[] recFolderFile;
    public StreamInfo si;
    public TimeShiftConfig timeShiftConfig;
    public string userId;
    public string[] webSocketRecInfo;
    ...
    public void set(bool isChaseCheck, config.Config cfg, MainForm form)
    {
        ...
        webSocketRecInfo = getWebSocketInfo(si.data, isRtmp, isChase, si.isTimeShift, form, isFmp4);
    ...
    public string[] getRecFilePath(bool isRtmp, TimeShiftConfig timeShiftConfig, bool isFmp4, config.Config cfg,
        MainForm form)
    {
        ...
        try
        {
            ...
            return util.getRecFolderFilePath(si.recFolderFileInfo[0], si.recFolderFileInfo[1],
                si.recFolderFileInfo[2], id, si.recFolderFileInfo[4], si.recFolderFileInfo[5], cfg, si.isTimeShift,
                timeShiftConfig, si.openTime, isRtmp, isFmp4);
        }
        catch (Exception e)
        {
    ...
    private bool isChaseRec(bool isChaseCheck, bool isChasable, bool isReservation, string data, config.Config cfg)
    {
...
//			util.debugWriteLine(data);
        ...
        if (ver == "1")
        {
            ...
            if (isRtmp && !isTimeShift)
                request =
                    "{\"type\":\"watch\",\"body\":{\"command\":\"getpermit\",\"requirement\":{\"broadcastId\":\"" +
                    broadcastId +
                    "\",\"route\":\"\",\"stream\":{\"protocol\":\"rtmp\",\"requireNewStream\":true},\"room\":{\"isCommentable\":true,\"protocol\":\"webSocket\"}}}}";
            else
                request = //(isChase) ? 
                    ...
                    "{\"type\":\"watch\",\"body\":{\"command\":\"getpermit\",\"requirement\":{\"broadcastId\":\"" +
                    broadcastId +
                    "\",\"route\":\"\",\"stream\":{\"protocol\":\"hls\",\"requireNewStream\":true,\"priorStreamQuality\":\"normal\", \"isLowLatency\": false,\"isChasePlay\":false},\"room\":{\"isCommentable\":true,\"protocol\":\"webSocket\"}}}}";
        }
        else if (ver == "2")
        {
            request = isRtmp
                ? "{\"type\":\"startWatching\",\"data\":{\"stream\":{\"quality\":\"normal\",\"protocol\":\"rtmp\",\"latency\":\"high\",\"chasePlay\":false},\"room\":{\"protocol\":\"webSocket\",\"commentable\":true},\"reconnect\":false}}"
                : "{\"type\":\"startWatching\",\"data\":{\"stream\":{\"quality\":\"normal\",\"protocol\":\"hls" +
                  (isFmp4 ? "+fmp4" : "") + "\",\"latency\":\"" +
                  (latency == "新方式の低遅延" || latency == "1" ? "low" : "high") + "\",\"chasePlay\":" +
                  (isChase ? "true" : "false") +
...
//			string request = "{\"type\":\"watch\",\"body\":{\"command\":\"getpermit\",\"requirement\":{\"broadcastId\":\"" + broadcastId + "\",\"route\":\"\",\"stream\":{\"protocol\":\"rtmp\"},\"room\":{\"isCommentable\":true,\"protocol\":\"webSocket\"}}}}";
        ...
        return new[] { wsUrl, request, ver, broadcastId };
    }
...
//						if (rm.ri != null) timeShiftConfig = rm.ri.tsConfig;
            ...
            {
                timeShiftConfig = getReadyArgTsConfig(rm.argTsConfig.clone(), si.recFolderFileInfo[0],
                    si.recFolderFileInfo[1], si.recFolderFileInfo[2], si.recFolderFileInfo[3],
                    si.recFolderFileInfo[4], si.recFolderFileInfo[5], si.openTime,
                    (int)(si.openTime - si._openTime), rm);
            }
            else
            {
                timeShiftConfig = getTimeShiftConfig(si.recFolderFileInfo[0], si.recFolderFileInfo[1],
                    si.recFolderFileInfo[2], si.recFolderFileInfo[3], si.recFolderFileInfo[4],
                    si.recFolderFileInfo[5], rm.cfg, si.openTime, isChase, si._openTime, rm);
        ...
        try
        {
            ...
            try
            {
                rm.form.formAction(() =>
                {
                    ...
                    {
                        util.debugWriteLine("timeshift option form invoke " + e.Message + " " + e.StackTrace + " " +
            ...
            {
                util.debugWriteLine("timeshift option form invoke try " + e.Message + " " + e.StackTrace + " " +
                                    e.Source + " " + e.TargetSite);
            }
        ...
        long _openTime, int prepTime, RecordingManager rm)
    {
        ...
        util.debugWriteLine("ready arg ts iscontinueconcat " + _tsConfig.isContinueConcat + " startType " +
                            _tsConfig.timeType + " lastfiletime " + lastFileTime + " lastfile " + lastFile);
    ...
    public bool setRecFolderFile(RecordingManager rm)
    {
        ...
        {
            var fName = "a/" + util.getFileName(si.recFolderFileInfo[0], si.recFolderFileInfo[1],
                si.recFolderFileInfo[2], si.recFolderFileInfo[3], si.recFolderFileInfo[4], rm.cfg, si.openTime);
            recFolderFile = new[] { fName, fName, fName }; //new string[]{"", "", ""};
        }

        for (var i = 0; i < recFolderFile.Length; i++)
    ...
    {
        var ri = (RecordInfo)MemberwiseClone();
        ri.si = si.clone();
        return ri;
    }
}